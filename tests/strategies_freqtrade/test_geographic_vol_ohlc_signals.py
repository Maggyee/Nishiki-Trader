...
def test_v18_generators_emit_only_state_changes_and_long_flat(strategy: str) -> None:
...
def test_v18_fold_starts_flat_and_emits_first_eligible_buy() -> None:
    events = generate_geographic_vol_signals(
...
def test_v18_parameters_cannot_be_tuned() -> None:
    with pytest.raises(ValueError, match="cannot be tuned"):
        VolatilityReliefParams(change_observations=10)


def test_v18_session_audit_rejects_publication_lookahead() -> None:
