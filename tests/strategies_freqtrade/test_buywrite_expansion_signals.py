...
from apps.strategies_freqtrade.research.buywrite_expansion_signals import (
    audit_factor_frame,
    generate_buywrite_expansion_signals,
)
...
def test_generate_buywrite_expansion_signals_emits_buy_and_flat() -> None:
    ...
    assert all(e.source == "rule_cboe_bxn_expansion_v1" for e in events)


def test_generate_buywrite_expansion_signals_rejects_unknown_candidate() -> None:
    df = _make_dummy_frame()
    with pytest.raises(ValueError, match="unsupported candidate"):
        generate_buywrite_expansion_signals(df, candidate="unknown_cand")
