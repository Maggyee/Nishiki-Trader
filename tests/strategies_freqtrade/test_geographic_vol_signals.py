...
from apps.strategies_freqtrade.research.geographic_vol_signals import (
    STRATEGY_IDENTITIES,
...
def test_v17_generators_emit_only_state_changes_and_long_flat(strategy: str) -> None:
...
def test_v17_fold_starts_flat_and_emits_first_eligible_buy() -> None:
    events = generate_geographic_vol_signals(
        _frame([30, 29, 28, 27, 26, 25, 24, 23]),
        strategy="em_vol_relief",
...
def test_v17_parameters_cannot_be_tuned() -> None:
    with pytest.raises(ValueError, match="cannot be tuned"):
        VolatilityReliefParams(change_observations=10)


def test_v17_session_audit_rejects_publication_lookahead() -> None:
