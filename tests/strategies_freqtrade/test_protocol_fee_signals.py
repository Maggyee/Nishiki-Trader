...
from apps.strategies_freqtrade.research.protocol_fee_signals import (
    generate_protocol_fee_expansion_signals,
)
...
def test_protocol_fee_expansion_rules_emit_buy_then_flat(candidate: str) -> None:
...
def test_protocol_fee_expansion_rule_rejects_lag_drift() -> None:
    ...
    with pytest.raises(ValueError, match="two-day publication lag"):
        generate_protocol_fee_expansion_signals(
            frame,
            candidate="all_revenue_expansion",
            start_date="2019-11-01",
        )
