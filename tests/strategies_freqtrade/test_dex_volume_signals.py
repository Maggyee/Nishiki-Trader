...
from apps.strategies_freqtrade.research.dex_volume_signals import (
    generate_dex_volume_expansion_signals,
)
...
def test_dex_volume_expansion_rules_emit_buy_then_flat(candidate: str) -> None:
...
def test_dex_volume_expansion_rule_rejects_lag_drift() -> None:
    ...
    with pytest.raises(ValueError, match="two-day publication lag"):
        generate_dex_volume_expansion_signals(
            frame,
            candidate="ethereum_dex_volume_expansion",
            start_date="2019-11-01",
        )
