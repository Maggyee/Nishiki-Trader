...
from apps.strategies_freqtrade.research.size_style_vol_signals import (
    STRATEGY_IDENTITIES,
    VolatilityReliefParams,
    audit_session_point_in_time_frame,
    generate_size_style_vol_signals,
)
...
def test_v19_generators_emit_only_state_changes_and_long_flat(strategy: str) -> None:
...
def test_v19_fold_starts_flat_and_emits_first_eligible_buy() -> None:
    events = generate_size_style_vol_signals(
        _frame([30, 29, 28, 27, 26, 25, 24, 23]),
        strategy="russell_vol_relief",
...
def test_v19_parameters_cannot_be_tuned() -> None:
    with pytest.raises(ValueError, match="cannot be tuned"):
        VolatilityReliefParams(change_observations=10)


def test_v19_session_audit_rejects_publication_lookahead() -> None:
