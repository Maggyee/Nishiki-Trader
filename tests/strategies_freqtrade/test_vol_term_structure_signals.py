...
def test_generate_vol_term_structure_signals_emits_buy_and_flat() -> None:
    df = _make_dummy_frame(25)
    events = generate_vol_term_structure_signals(
        df,
        candidate="vix3m_relief",
        start_date="2020-01-01",
        end_date="2020-01-20",
    )
    assert len(events) > 0
    assert all(e.side in {"buy", "flat"} for e in events)
    assert all(e.source == "rule_cboe_vix3m_relief_v1" for e in events)


def test_generate_vol_term_structure_signals_rejects_unknown_candidate() -> None:
    df = _make_dummy_frame()
    with pytest.raises(ValueError, match="unsupported candidate"):
        generate_vol_term_structure_signals(df, candidate="unknown_cand")
