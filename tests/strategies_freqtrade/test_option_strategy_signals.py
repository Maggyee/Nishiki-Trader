...
def test_generate_option_strategy_signals_emits_buy_and_flat() -> None:
    ...
    assert all(e.source == "rule_cboe_vpn_expansion_v1" for e in events)


def test_generate_option_strategy_signals_rejects_unknown_candidate() -> None:
    df = _make_dummy_frame()
    with pytest.raises(ValueError, match="unsupported candidate"):
        generate_option_strategy_signals(df, candidate="unknown_cand")
