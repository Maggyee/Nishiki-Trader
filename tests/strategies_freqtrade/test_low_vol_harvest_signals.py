...
from apps.strategies_freqtrade.research.low_vol_harvest_signals import (
    audit_factor_frame,
    generate_low_vol_harvest_signals,
)
...
def test_generate_low_vol_harvest_signals_emits_buy_and_flat() -> None:
    ...
    assert all(e.source == "rule_cboe_lovol_expansion_v1" for e in events)


def test_generate_low_vol_harvest_signals_rejects_unknown_candidate() -> None:
    df = _make_dummy_frame()
    with pytest.raises(ValueError, match="unsupported candidate"):
        generate_low_vol_harvest_signals(df, candidate="unknown_cand")
