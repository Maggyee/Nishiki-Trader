...
from apps.ops.research_v18_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def _history(_kind: str = "vxeem") -> bytes:
...
def test_v18_dry_run_has_no_network_or_write() -> None:
...
def test_v18_snapshot_round_trip_and_factor_lag(tmp_path: Path) -> None:
    snapshot_path, result = collect_snapshot(
        "vxefa",
        output_dir=tmp_path / "raw",
        fetch=lambda _: _history("vxefa"),
...
def test_v18_snapshot_rejects_tampering(tmp_path: Path) -> None:
    path, _ = collect_snapshot(
        "vxn",
        output_dir=tmp_path,
        fetch=lambda _: _history("vxn"),
        now=datetime(2026, 8, 13, 2, 30, tzinfo=UTC),
    )
...
def test_v18_snapshot_rejects_wrong_header(tmp_path: Path) -> None:
