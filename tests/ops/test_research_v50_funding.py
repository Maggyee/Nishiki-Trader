"""Offline tests for Research Protocol v50 (no network, synthetic funding data)."""
...
from apps.ops.research_v50_funding import (
    _apply_gates,
...
def test_contract_is_valid_and_fingerprint_stable() -> None:
    ...
    assert set(IDENTITIES) == {"fund_neg_3d", "fund_below_baseline_3d", "fund_overheat_flat_3d"}
    assert WINDOWS["development"][1] < WINDOWS["confirmation"][0]
...
def test_candidate_states_follow_frozen_rules_and_cutoff() -> None:
    ...
    # 2020-01-08..2020-01-10: nine settlements of -0.0002 (negative crowding)
    for day in ("2020-01-08", "2020-01-09", "2020-01-10"):
        for hour in ("00:00", "08:00", "16:00"):
            stamps[f"{day} {hour}"] = -0.0002
    # 2020-01-11..12: strongly positive (overheated)
    for day in ("2020-01-11", "2020-01-12"):
        for hour in ("00:00", "08:00", "16:00"):
            stamps[f"{day} {hour}"] = 0.001
    states = build_candidate_states(closes, _funding_series(stamps))
    day0 = 0  # 2020-01-10: trailing 72h is all negative
...
def test_candidate_states_ignore_settlements_after_cutoff() -> None:
    ...
    # next-day settlement (after the day-10 cutoff) must not affect day 10
    with_future["2020-01-11 00:00"] = 1.0
    states_a = build_candidate_states(closes, _funding_series(base))
    states_b = build_candidate_states(closes, _funding_series(with_future))
...
def test_candidate_states_force_flat_when_settlements_missing() -> None:
    ...
    states = build_candidate_states(closes, sparse)
    assert int(PARAMETERS["min_settlements_in_window"]) > 2
    for key in IDENTITIES:
...
def test_funding_quality_gates_fail_closed() -> None:
    ...
    errors = funding_quality_errors(report, min_settlements=10)
    ...
    assert any("sanity" in e for e in funding_quality_errors(report, min_settlements=1))
...
def test_apply_gates_requires_all_checks() -> None:
    ...
    good = _apply_gates("confirmation", candidate, bh_pnl=10.0, p_value=0.05)
    assert good["pass_gates"] is True
    assert good["benchmark_floor"] == pytest.approx(5.0)
    bad = _apply_gates("confirmation", candidate, bh_pnl=30.0, p_value=0.05)
