...
from apps.ops.research_v31_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def _history(kind: str, *, schema: str = "scalar") -> bytes:
    index = {"vxslv": "VXSLV", "vxxle": "VXXLE", "vxtlt": "VXTLT"}[kind]
...
def test_v31_dry_run_has_no_network_or_write() -> None:
    plan = request_plan("vxslv")

    assert plan["network_accessed"] is False
    assert plan["data_written"] is False
    assert plan["url"].endswith("VXSLV_History.csv")


@pytest.mark.parametrize(("kind", "schema"), [("vxslv", "scalar"), ("vxtlt", "ohlc")])
def test_v31_snapshot_round_trip_and_factor(tmp_path: Path, kind: str, schema: str) -> None:
...
def test_v31_snapshot_rejects_unknown_header(tmp_path: Path) -> None:
...
def test_v31_snapshot_rejects_tampering(tmp_path: Path) -> None:
    path, _ = collect_snapshot("vxslv", output_dir=tmp_path, fetch=lambda _: _history("vxslv"))
