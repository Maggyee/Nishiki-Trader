...
from apps.ops.research_v45_snapshot import (
    collect_and_qualify_all,
    compute_factors,
)


def test_compute_factors_v45_generates_three_series() -> None:
    rows = []
    for i in range(100):
        rows.append({
    ...
    assert "btc_prem_diff5_tight" in factors
    assert "btc_prem_diff4_negative" in factors
    assert len(factors["btc_prem_diff5_negative"]) == 100


def test_collect_and_qualify_all_structure_v45() -> None:
