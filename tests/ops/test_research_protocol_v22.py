...
def test_protocol_v22_is_frozen_and_valid() -> None:
...
def test_protocol_v22_rejects_direction_drift() -> None:
    payload = json.loads(DEFAULT_PROTOCOL.read_text())
    drifted = copy.deepcopy(payload)
    drifted["candidates"][2]["parameters"]["direction"] = "negative"

    with pytest.raises(ValueError, match="candidate parameters drifted"):
        validate_protocol(drifted)


def test_protocol_v22_rejects_opened_history() -> None:
    payload = json.loads(DEFAULT_PROTOCOL.read_text())
