...
from apps.ops.research_v32_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def _history(kind: str, *, schema: str = "scalar") -> bytes:
    index = {"euvix": "EUVIX", "jyvix": "JYVIX", "bpvix": "BPVIX"}[kind]
...
def test_v32_dry_run_has_no_network_or_write() -> None:
    plan = request_plan("euvix")

    assert plan["network_accessed"] is False
    assert plan["data_written"] is False
    assert plan["url"].endswith("EUVIX_History.csv")


@pytest.mark.parametrize(("kind", "schema"), [("euvix", "scalar"), ("bpvix", "ohlc")])
def test_v32_snapshot_round_trip_and_factor(tmp_path: Path, kind: str, schema: str) -> None:
...
def test_v32_snapshot_rejects_unknown_header(tmp_path: Path) -> None:
...
def test_v32_snapshot_rejects_tampering(tmp_path: Path) -> None:
    path, _ = collect_snapshot("euvix", output_dir=tmp_path, fetch=lambda _: _history("euvix"))
