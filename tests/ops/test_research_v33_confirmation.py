...
from apps.ops.research_v33_confirmation import DEFAULT_CONTRACT, validate_contract
from apps.ops.research_v33_confirmation_factor import _audit_confirmation_rows
from apps.ops.research_v33_confirmation_review import _classification
...
def test_v33_confirmation_contract_is_valid() -> None:
...
def test_v33_confirmation_rejects_drift(path: tuple, value: object) -> None:
...
def _rows(*, omitted: set[date] | None = None) -> list[dict[str, object]]:
    ...
    while current <= end:
        if current.weekday() < 5 and current not in (omitted or set()):
            values.append({"date": current.isoformat(), "value": 50.0})
        current += timedelta(days=1)
    return values


def test_v33_confirmation_factor_audit_accepts_complete_grid() -> None:
...
def test_v33_confirmation_factor_audit_rejects_large_gap() -> None:
    omitted = {
        date(2024, 1, 1),
        date(2024, 1, 2),
        date(2024, 1, 3),
...
def test_v33_confirmation_classification() -> None:
