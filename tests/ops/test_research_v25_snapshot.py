...
from apps.ops.research_v25_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def _history(*, extra_2023: bool = False) -> bytes:
    ...
    while current <= last:
        ...
        items.append({"date": int(midnight.timestamp()), "tvl": value})
        value += 1_000.0
        current += timedelta(days=1)
    return json.dumps(items).encode()


def test_v25_dry_run_has_no_network_or_write() -> None:
    plan = request_plan("ethereum")

    assert plan["network_accessed"] is False
    assert plan["values_reported"] is False
    assert plan["url"].endswith("/historicalChainTvl/Ethereum")


def test_v25_snapshot_round_trip_and_factor(tmp_path: Path) -> None:
...
def test_v25_snapshot_rejects_tampering(tmp_path: Path) -> None:
    path, _ = collect_snapshot("bitcoin", output_dir=tmp_path, fetch=lambda _: _history())
