...
from apps.ops.research_v19_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def _history(_kind: str = "rvx") -> bytes:
...
def test_v19_dry_run_has_no_network_or_write() -> None:
    plan = request_plan("rvx")
    assert plan["network_accessed"] is False
    assert plan["data_written"] is False
    assert plan["boundaries"]["credentials_loaded"] is False
    assert plan["url"].endswith("RVX_History.csv")


def test_v19_snapshot_round_trip_and_factor_lag(tmp_path: Path) -> None:
    snapshot_path, result = collect_snapshot(
        "vxd",
        output_dir=tmp_path / "raw",
        fetch=lambda _: _history("vxd"),
...
def test_v19_snapshot_rejects_tampering(tmp_path: Path) -> None:
    path, _ = collect_snapshot(
        "vxfxi",
        output_dir=tmp_path,
        fetch=lambda _: _history("vxfxi"),
        now=datetime(2026, 8, 13, 3, 30, tzinfo=UTC),
    )
...
def test_v19_snapshot_rejects_wrong_header(tmp_path: Path) -> None:
