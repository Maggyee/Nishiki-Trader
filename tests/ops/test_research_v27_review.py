...
DEVELOPMENT_RESULTS = Path("docs/progress/phase-2-research-v27-development-results.json")
...
def test_v27_development_results_reject_all_three_identities() -> None:
    payload = json.loads(DEVELOPMENT_RESULTS.read_text())

    assert payload["schema_version"] == "research.v27.development_results.v1"
    assert payload["development_passer_count"] == 0
    assert payload["confirmation_open_eligible_candidates"] == []
    assert payload["recommendation"] == "stop_protocol_v27_no_confirmation_open"
