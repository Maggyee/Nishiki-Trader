...
from apps.ops.research_v37_snapshot import (
    collect_and_qualify_all,
    parse_and_audit_csv,
)

SAMPLE_CSV = b"""Date,Close
01/02/2020,100.0
01/03/2020,101.5
01/06/2020,102.0
"""
...
def test_parse_and_audit_csv() -> None:
    # 500 rows needed for full audit, test parsing logic on repeated rows
    header = b"Date,Close\n"
    lines = [f"01/{i:02d}/2020,100.{i}\n".encode() for i in range(1, 29)] * 20
    payload = header + b"".join(lines)
    rows, audit = parse_and_audit_csv("bxn", payload)
    assert len(rows) == 560
    assert audit["kind"] == "bxn"
...
def test_collect_and_qualify_all_mocked(tmp_path: Path) -> None:
    ...
    # construct mock data with > 700 rows spanning 2019-11-01 to 2022-12-31
    import pandas as pd
    ...
    assert set(qual["sources"]) == {"bxn", "bxy", "bxr"}
    assert qual_path.exists()
    payload = json.loads(qual_path.read_text())
    assert payload["schema_version"] == "research.provider_qualification.v37.v1"
