...
from apps.ops.research_v14_snapshot import collect_snapshot, verify_snapshot
            ...
            rows.append(
                {
                    ...
                    "CapRealUSD": str(value * 1_000_000),
                    "NVTAdj": str(value),
                    "SOPR": "1.01",
                }
            )
...
def test_v14_snapshot_round_trip(tmp_path: Path) -> None:
...
def test_v14_snapshot_rejects_incomplete_grid(tmp_path: Path) -> None:
...
def test_v14_snapshot_rejects_tampering(tmp_path: Path) -> None:
