...
from apps.ops.research_v26_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def test_v26_dry_run_has_no_network_or_write() -> None:
    plan = request_plan("ethereum")

    assert plan["network_accessed"] is False
    assert plan["values_reported"] is False
    assert "/overview/dexs/ethereum" in plan["url"]


def test_v26_snapshot_round_trip_and_factor(tmp_path: Path) -> None:
    path, result = collect_snapshot(
        "all_dexs",
        output_dir=tmp_path / "raw",
        fetch=lambda _: _history(extra_2023=True),
        now=datetime(2026, 8, 13, 7, 15, tzinfo=UTC),
    )
    factor_path = tmp_path / "volume.csv"
...
def test_v26_snapshot_rejects_tampering(tmp_path: Path) -> None:
    path, _ = collect_snapshot("solana", output_dir=tmp_path, fetch=lambda _: _history())
