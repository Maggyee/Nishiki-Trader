...
from apps.ops.research_v7_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def test_v7_dry_run_has_no_network_or_write() -> None:
    ...
    assert plan["url"].endswith("VIX_History.csv")


def test_v7_snapshot_round_trip_and_factor_lag(tmp_path: Path) -> None:
    ...
    assert int(first["ts_event"]) == int(
        datetime(2019, 11, 2, tzinfo=UTC).timestamp() * 1_000_000_000
    )


def test_v7_snapshot_rejects_tampering(tmp_path: Path) -> None:
...
def test_v7_snapshot_rejects_wrong_header(tmp_path: Path) -> None:
