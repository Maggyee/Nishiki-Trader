...
from apps.ops.research_v34_snapshot import (
    collect_snapshot,
    request_plan,
    verify_snapshot,
    write_factor_csv,
...
def _history(kind: str, *, schema: str = "scalar") -> bytes:
    index = {"fvx": "FVX", "tnx": "TNX", "tyx": "TYX"}[kind]
    ...
    while current <= date(2022, 12, 31):
        if current.weekday() < 5:
            if schema == "scalar":
                ...
                lines.append(
                    f"{current.strftime('%m/%d/%Y')},{value:.4f},{value + 0.1:.4f},{value - 0.1:.4f},{value + 0.05:.4f}"
                )
            value += 0.001
...
def test_v34_dry_run_has_no_network_or_write() -> None:
    plan = request_plan("tnx")

    assert plan["network_accessed"] is False
    assert plan["data_written"] is False
    assert plan["url"].endswith("TNX_History.csv")


@pytest.mark.parametrize(("kind", "schema"), [("fvx", "scalar"), ("tnx", "ohlc")])
def test_v34_snapshot_round_trip_and_factor(tmp_path: Path, kind: str, schema: str) -> None:
    path, result = collect_snapshot(
        kind,
        output_dir=tmp_path / "raw",
        fetch=lambda _: _history(kind, schema=schema),
        now=datetime(2026, 8, 14, 1, 44, tzinfo=UTC),
    )
...
def test_v34_snapshot_rejects_unknown_header(tmp_path: Path) -> None:
...
def test_v34_snapshot_rejects_tampering(tmp_path: Path) -> None:
    path, _ = collect_snapshot("tnx", output_dir=tmp_path, fetch=lambda _: _history("tnx"))
