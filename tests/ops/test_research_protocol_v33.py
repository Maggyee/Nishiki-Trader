...
def test_protocol_v33_is_frozen_and_valid() -> None:
...
def test_protocol_v33_rejects_direction_drift() -> None:
    payload = json.loads(DEFAULT_PROTOCOL.read_text())
    drifted = copy.deepcopy(payload)
    drifted["candidates"][0]["parameters"]["direction"] = "positive"

    with pytest.raises(ValueError, match="candidate parameters drifted"):
        validate_protocol(drifted)


def test_protocol_v33_rejects_opened_history() -> None:
    payload = json.loads(DEFAULT_PROTOCOL.read_text())
