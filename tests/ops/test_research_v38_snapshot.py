...
from apps.ops.research_v38_snapshot import (
...
def test_parse_and_audit_csv() -> None:
    ...
    rows, audit = parse_and_audit_csv("vpd", payload)
    assert len(rows) == 560
    assert audit["kind"] == "vpd"
...
def test_collect_and_qualify_all_mocked(tmp_path: Path) -> None:
    ...
    assert set(qual["sources"]) == {"vpd", "pput", "cll"}
    assert qual_path.exists()
    payload = json.loads(qual_path.read_text())
    assert payload["schema_version"] == "research.provider_qualification.v38.v1"
