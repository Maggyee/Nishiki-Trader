...
- **Current objective**: Phase 5 remains active and `stop_before_testnet_resume` remains in force. The original 16-candidate registry and Protocol v2-v5 research identities are frozen.
  ...
  **Protocol v50 (B2, BTC funding-rate positioning states, family `crypto_derivatives_structure`) is pre-registered and frozen** under Gates v2: three candidates with structural (non-tuned) thresholds — 72h funding sum < 0 (contrarian short-crowding), 72h mean < 0.01%/8h baseline, and overheat-flat above 5× baseline — on Binance Vision USD-M fundingRate archives with a staged fetch boundary (confirmation months 2023-01..2025-12 may not be fetched before a development pass, enforced in code). Dev window 2020-2022, ≥18/36 breadth, full Gates v2; among dev passers only the best base-PnL candidate opens 2023-2025 confirmation. Contract `apps/ops/research_protocol_v50.py`; pipeline `apps/ops/research_v50_funding.py`.
