...
- **Current objective**: ADR-008 Phase 3b connection probe ran end-to-end on Binance Spot testnet with Ed25519 credentials, DataClient + ExecClient + UserData WS all connected, ExecutionMassStatus reconciliation succeeded, no orders submitted (zero strategies / zero actors), bundle landed under `data/testnet/`. Next is ADR-008 §6.3 (emergency_flatten + kill-switch). `freqai_linear_v1` stays on `paper_simulated`; the `phase_3_not_ready` promotion gate stays closed.
...
3. ADR-008 §6.2 Phase 3b is now feature-complete and live-verified (bundle `20260518-122839Z-24bf3db2`, retro `2026-05-18-phase-3b-testnet-connect-probe.md`). Phase 3 work moves to §6.3 (emergency_flatten + kill-switch) next; the `phase_3_not_ready` promotion gate stays closed.
...
2. Start ADR-008 §6.3: `apps/strategies_nautilus/runners/emergency_flatten.py` with the seven-step `cancel → wait → reduce-only flatten → audit` flow, `emergency_flatten.json` sidecar, `--cancel-only` and `--reduce-only` modes, timeout-residual reporting. Drive it with a fake exchange client in unit tests so no network is required. Then wire ADR-008 §5.2 auto-triggers (5% daily loss kill-switch, exchange_error / ws_reconnect bursts) into `testnet_runner.py`'s eventual long-running path.
...
On 2026-05-18, after the Phase 3b connection probe was implemented, unit-tested, and live-verified end-to-end against Binance Spot testnet:
