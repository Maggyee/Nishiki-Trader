...
- **Current objective**: Phase 5 remains active and `stop_before_testnet_resume` remains in force. The original 16-candidate registry and Protocol v2-v5 research identities are frozen.
  ...
  **Protocol v49 (B1, `freqai_panel_ridge_v1 / panel17-ridge-mwf-oos2020h2-v1`, family `multifactor_ml`) is pre-registered and frozen** under Gates v2: 17 archived factor snapshot pairs SHA256-pinned in `apps/ops/research_protocol_v49.py`, ridge monthly walk-forward, dev OOS 2020-07..2022-12 then one-shot 2023-2025 confirmation; pipeline `apps/ops/research_v49_multifactor.py` (qualify → develop → confirm). Pre-freeze data reads were metadata only (hashes/headers/coverage). A confirmation failure closes the identity permanently.
