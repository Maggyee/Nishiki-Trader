...
- **Status file**: Active
- **Last updated**: 2026-08-13 (Protocol v18 confirmed VXN relief is held at identity-specific `paper_shadow` dry run)
...
- `docs/progress/phase-2-research-protocol-v18.md` — OHLC recovery of the
  unchanged v17 rules; confirmed VXN is held at `paper_shadow` dry run.
...
14. Preserve v17's schema mismatch, the v18 VXEEM/VXEFA rejects, and confirmed VXN at its reviewed `paper_shadow` policy. Do not retune, disable dry-run, enter `paper_simulated`, or open the future blind. A prospective collector must be implemented/reviewed before any operator-approved schedule installation.
...
18. For v18 VXN relief, keep `dry_run=True` and multiplier `0.2`. The next implementation step is a fail-closed prospective VXN/BTC collector; review it before use and install no recurring schedule without explicit operator approval. Accumulate 7 qualified days or 50 new signals before any separate human `paper_simulated` review.
...
- Forward evidence is 0/7 days and 0/50 new signals; no collector/schedule was
  installed. `paper_simulated`, future blind, testnet, and live remain closed.
