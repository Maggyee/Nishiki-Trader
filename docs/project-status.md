...
On 2026-07-01, after adding saved readiness-report artifact fingerprints to the Phase 6 startup evidence chain:

- `phase6.live_startup_guard.v1` now records `evidence.live_readiness_report.sha256`, the SHA-256 of the saved `phase6.live_readiness.v1` JSON artifact the startup guard actually consumed.
- `dashboard.snapshot.v1` startup-guard summaries now include a read-only `Readiness report artifact` evidence row, and mark old startup-guard artifacts as blocked when they claim `startup_allowed=true` without that readiness artifact fingerprint.
- ADR-013 and `docs/runbook-first-live-day.md` now document the startup-guard consumed-readiness-artifact fingerprint check.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **41 passed**.
- `UV_CACHE_DIR=/tmp/uv-cache uv run ruff check apps tests docs infra` -> clean.
- `git diff --check` -> clean.
- Current blocked startup-guard smoke with a saved readiness report -> exit **2**, `startup_allowed=false`, `live_trading_authorized=false`, `places_orders=false`, and `evidence.live_readiness_report.sha256` matches the SHA-256 of the saved readiness JSON file.
- Dashboard snapshot smoke with those blocked Phase 6 artifacts -> `phase6.state=blocked`, startup report `status=blocked`, `authorizes_live_trading=false`, and `Readiness report artifact` evidence `status=ok` with the same SHA-256.
