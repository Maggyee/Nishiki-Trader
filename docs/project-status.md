...
- **Current objective**: Continue ADR-008 Phase 3b from guarded Binance Spot testnet adapter configuration toward a controlled testnet connection probe, while keeping `freqai_linear_v1` on `paper_simulated` and preserving the credential/order-path gates.
...
3. Continue Phase 3b behind startup gates only. `testnet_runner.py` now builds Binance Spot TESTNET adapter config after validation, but does not connect to Binance testnet yet.
...
2. Add the first controlled Binance Spot testnet connection probe behind `testnet_runner.py`'s existing gates: instantiate/connect only after validation, write `data/testnet/<run_id>/logs/runtime.log` with redacted `credentials_loaded`, and exit before any strategy can submit orders.
3. Keep the `testnet_runner.py` startup guard as the first line of defense: explicit `--allow-real-credentials`, clean git, source/model retro evidence, testnet multiplier cap, key-prefix-only audit, and Binance Spot TESTNET-only adapter config before any network connection is opened.
...
On 2026-05-18, after the Phase 3a 24h wall-clock soak completed and the retro was recorded:

- `UV_CACHE_DIR=/tmp/uv-cache uv run pytest -q` -> 319 passed.
...
- `tests/strategies_nautilus/test_testnet_runner_startup.py` enforces the Phase 3b startup boundary: `--allow-real-credentials` required, credential lengths checked, dirty git rejected, paper_simulated/testnet retro evidence required, multiplier capped at 0.2, non-Binance / non-Spot configs rejected, Binance Spot TESTNET adapter config built without embedding credentials, JSON output redacts full key/secret, and no exchange connection or bundle write is performed.
