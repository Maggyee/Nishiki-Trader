...
On 2026-07-01, after binding passive Phase 6 readiness to live promotion-review artifact fingerprints:

- `apps.strategies_nautilus.runners.promotion_review_artifact` now records a SHA-256 fingerprint for the exact `promotion_review.py` JSON or Markdown artifact bytes after exact live-canary field parsing.
- `phase6.live_readiness.v1` now includes the evaluated `live_promotion_review` gate, including `sha256` when an artifact is supplied.
- `phase6.live_startup_guard.v1` now rejects saved readiness reports whose `live_promotion_review.accepted` is not true or whose recorded `live_promotion_review.sha256` differs from the promotion artifact supplied at startup, even if the replacement artifact's structured fields are otherwise valid.
- ADR-013 and `docs/runbook-first-live-day.md` now document the readiness-report-to-startup promotion artifact fingerprint check.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/strategies_nautilus/test_promotion_review.py tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py -q` -> **52 passed**.
- `UV_CACHE_DIR=/tmp/uv-cache uv run ruff check apps tests docs infra` -> clean.
- Current blocked readiness smoke with `--market-type spot --max-leverage 1` -> `readiness_gate_met=false`, `live_trading_allowed=false`, `git.dirty=true`, `places_orders=false`, `live_promotion_review.accepted=false`, and blockers include `git_dirty`, missing live promotion, ADR-013 Draft, and testnet continuity evidence missing.
- Current startup-guard smoke with that fresh but blocked readiness report and `--max-readiness-report-age-seconds 86400` -> exit **2**, `startup_allowed=false`, `live_trading_authorized=false`, `readiness_fresh=true`, `live_promotion_review_not_found`, `live_promotion_review_sha256` in readiness problems, and `places_orders=false`.
