...
On 2026-07-02, after tightening Phase 6 dashboard handling for source/model
identity and startup-readiness artifact evidence:
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **90 passed**.
...
- Blocked readiness/startup/dashboard smoke with an attached testnet bundle ->
  ...
  `live_trading_authorized=false`, the startup-side readiness artifact row
  stayed `blocked` with `live_readiness_report_gate_not_met`,
  `phase6.state=blocked`, `phase6.boundaries.authorizes_live_trading=false`,
  and `phase6.boundaries.places_orders=false`; the dashboard keeps the current
  Draft ADR/runbook and other saved-report blockers visible without opening
  the live path.
