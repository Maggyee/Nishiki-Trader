...
2. Implement the ADR-008 Phase 3 roadmap one sub-phase at a time, starting with Phase 3a (wall-clock paper without any credentials). Each sub-phase must close out the matching §7 checklist before moving on. The `promotion_review` `phase_3_not_ready` gate stays closed until §6.1–§6.5 are all done; only Phase 3f flips the gate via a small explicit patch.
