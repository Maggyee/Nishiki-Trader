...
- **Status file**: Active
- **Last updated**: 2026-08-13 (Protocol v29 Wikipedia macro-attention provider-qualified; PnL unopened)
...
- **Current objective**: Phase 5 remains active and `stop_before_testnet_resume` remains in force. The original 16-candidate registry and Protocol v2-v5 research identities are frozen. Protocol v5 curve carry and BVOL relief are rejected and the 2026-08..12 blind remains sealed. The provider/timer review is complete: v5 and completed v2 timers are `inactive/disabled`, v5 collector status is `archived`, and a four-day post-window audit found zero new invocations. The retained v5 tree is 13 snapshots / 13 Parquets with zero conflicts; v2 `COMPLETE`, ledger, deployment identity and all recorded checksums remain unchanged. Dashboard renders the archive as a valid terminal state with no retry or operations issue. Independent Protocol v6 remains `blocked_provider_qualification` on the official 12-row grid, with historical development closed.
  ...
  Protocol v29 Wikipedia macro-attention relief is provider-qualified from freeze `4e15d2d`. Federal_Reserve, Inflation, and Recession each have 1,096 unfilled 2020-2022 observations, 61 warmup rows, and a one-day maximum gap. Confirmation timestamps were not requested. Signals and PnL remain unopened; two clean-git cash replays per identity are the next step.
...
- `docs/progress/phase-2-research-protocol-v29.md` — Wikipedia macro-attention
  relief; all three articles provider-qualified, PnL unopened.
...
28. From the Protocol v29 qualification HEAD, generate development signals and two clean-git cash replays per identity on the v7 downtime catalog. Review with `research_v29_review`. Do not GET again, retune, sign-flip, or ensemble. Confirmation stays sealed until a passer has its own frozen 2023-2025 contract.
...
- No Protocol v28 reopen. All three options/OI identities failed coverage
  ...
  remain sealed.
- No Protocol v29 confirmation, retry, sign flip, ensemble, or future-blind
  opening. Development PnL may be opened only from the qualification HEAD.

## Latest Verification

On 2026-08-13, after Protocol v29 provider qualification:

- Each locked Wikimedia URL was opened once from freeze `4e15d2d`. All three
  articles qualified: 1,157 rows, 1,096 development observations, 61 warmup
  rows, maximum gap one day.
- Factors exist locally; signals and PnL remain unopened. Confirmation
  timestamps were not requested. Retry is forbidden.
