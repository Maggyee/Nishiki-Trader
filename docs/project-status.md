...
2. Draft the Phase 3 risk/runbook ADR (referenced as future work in ADR-007 §4): testnet runtime topology, real-exchange-credentials management (no creds in repo / .env layout), emergency-flatten / kill-switch workflow, restart recovery, and alerting. `freqai_linear_v1` is now `hold @ paper_simulated` in the project's first promote retro and the next stage `testnet_canary` is hard-blocked by `promotion_review`'s `phase_3_not_ready` gate until this ADR lands.
...
On 2026-05-17, after the project's first deliberate `promote` retro and the first paper_simulated bundle landed:
