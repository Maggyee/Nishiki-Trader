...
On 2026-07-01, after hardening passive Phase 6 promotion-review parsing:

- `apps.strategies_nautilus.runners.promotion_review_artifact` now parses `promotion_review.py` JSON or Markdown fields for exact live-canary promotion evidence.
- `phase6.live_readiness.v1` and `phase6.live_startup_guard.v1` now reject promotion-review artifacts unless they have exact source/model, `current_stage=testnet_canary`, `target_stage=live_canary`, `decision=promote`, `decision_allowed=yes`, a non-empty operator, and no review or promotion gate blockers.
- ADR-013 and `docs/runbook-first-live-day.md` now document that incidental stage mentions in rationale text do not count as signed live promotion evidence.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/strategies_nautilus/test_promotion_review.py tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py -q` -> **50 passed**.
...
- Current startup-guard smoke with that fresh but blocked readiness report and `--max-readiness-report-age-seconds 86400` -> exit **2**, `startup_allowed=false`, `live_trading_authorized=false`, `readiness_fresh=true`, `live_promotion_review_not_found`, and `places_orders=false`.
