...
13. Preserve the confirmed v16 Treasury-yield volatility identity unchanged at its reviewed `paper_shadow` policy. Its historical entry bundle and 92 collector baseline signals are state-seeding evidence only. Day 1 is qualified; do not run it repeatedly or install a recurring schedule without a separate operator decision. Do not disable dry-run, count baseline rows toward the forward gate, start testnet, or open the future blind.
...
17. For v16 Treasury-volatility relief, keep the reviewed policy at `dry_run=True`, multiplier `0.2`. Day 1 is qualified at 1/7 days and 0/50 signals. Decide separately whether to install the frozen weekday 12:30 UTC collector; crossing the OR-gate creates review eligibility rather than automatic `paper_simulated` authorization.
