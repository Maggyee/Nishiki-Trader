...
13. Preserve the confirmed v16 Treasury-yield volatility identity unchanged at its reviewed `paper_shadow` policy. Its historical entry bundle is dry-run plumbing evidence only. The prospective collector is frozen before its first request; run it once manually only after its implementation commit is clean and present on `origin/main`. A recurring schedule still requires separate operator approval. Do not disable dry-run, count historical rows toward the forward gate, start testnet, or open the future blind.
...
17. For v16 Treasury-volatility relief, keep the reviewed policy at `dry_run=True`, multiplier `0.2`. Commit and push the frozen Treasury/BTC collector, then run exactly one manual qualification attempt; only observations after `2026-08-12T09:37:17Z` may count toward 7 qualified days or 50 new signals. Do not install a schedule without separate approval, and remember that crossing the OR-gate creates review eligibility rather than automatic `paper_simulated` authorization.
