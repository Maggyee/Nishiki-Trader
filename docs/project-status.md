...
On 2026-07-01, after adding operator-document artifact fingerprints to the Phase 6 startup evidence chain:
...
- `dashboard.snapshot.v1` startup-guard summaries now include read-only `Live-risk ADR artifact`, `Readiness report artifact`, `Startup promotion review artifact`, `Readiness promotion SHA-256 match`, and `First live day runbook artifact` evidence rows, and mark old `startup_allowed=true` artifacts as blocked when required fingerprints are missing.
- ADR-013 and `docs/runbook-first-live-day.md` now document the startup-guard consumed-document fingerprint checks.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **42 passed**.
- `UV_CACHE_DIR=/tmp/uv-cache uv run ruff check apps tests docs infra` -> clean.
- `git diff --check` -> clean.
- Current blocked startup-guard smoke with a saved readiness report -> exit **2**, `startup_allowed=false`, `live_trading_authorized=false`, `places_orders=false`, and the ADR/readiness/runbook artifact SHA-256 values match their current file bytes.
- Dashboard snapshot smoke with those blocked Phase 6 artifacts -> `phase6.state=blocked`, startup report `status=blocked`, `authorizes_live_trading=false`, and evidence rows include `Live-risk ADR artifact`, `Readiness report artifact`, and `First live day runbook artifact`.
