...
On 2026-07-02, after tightening Phase 6 dashboard handling for startup-git
evidence:
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **87 passed**.
...
- Blocked readiness/startup/dashboard smoke with an attached testnet bundle ->
  ...
  `live_trading_authorized=false`, startup git evidence recorded the current
  in-progress dirty tree, `phase6.state=blocked`,
