...
2. Continue immutable daily July option/basis snapshots until the fixed paired coverage gate reaches 7/7 consecutive UTC days (current 1/7). Basis transformation is ready; option structural replication remains unimplemented. Keep stablecoin blocked until licensed `TxTfrValUSD` point-in-time access exists.
