...
On 2026-07-01, after adding source-document artifact fingerprints to the Phase 6 readiness evidence chain:
...
- `dashboard.snapshot.v1` readiness summaries now include read-only `Project status artifact`, `Readiness live-risk ADR artifact`, and `Promotion review artifact` evidence rows, and mark old passing readiness artifacts as blocked when required source-document fingerprints are missing.
- ADR-013 and `docs/runbook-first-live-day.md` now document the readiness source-document fingerprints and startup ADR fingerprint cross-check.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **52 passed**.
- `UV_CACHE_DIR=/tmp/uv-cache uv run ruff check apps tests docs infra` -> clean.
- `git diff --check` -> clean.
- Current blocked readiness/startup smoke -> startup guard exit **2**, `startup_allowed=false`, `live_trading_authorized=false`, `places_orders=false`, and project-status / live-risk ADR / readiness artifact SHA-256 values match their current file bytes.
- Dashboard snapshot smoke with those blocked Phase 6 artifacts -> `phase6.state=blocked`, both reports `status=blocked`, `authorizes_live_trading=false`, and evidence rows include `Project status artifact`, `Readiness live-risk ADR artifact`, `Live-risk ADR artifact`, and `Readiness report artifact`.
