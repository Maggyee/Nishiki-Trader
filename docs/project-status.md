...
- **Status file**: Active
- **Last updated**: 2026-07-01 (Phase 6 readiness freshness gate hardening)
...
On 2026-07-01, after hardening passive Phase 6 readiness freshness:

- `phase6.live_startup_guard.v1` now rejects saved readiness artifacts whose `generated_at_ns` is missing, in the future, or older than the configured maximum age; the default maximum age is 24 hours.
- ADR-013 and `docs/runbook-first-live-day.md` now document that saved readiness evidence must be fresh as well as generated from a clean git tree at the same commit as startup preflight.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py -q` -> **22 passed**.
- `UV_CACHE_DIR=/tmp/uv-cache uv run ruff check apps tests docs infra` -> clean.
- Current blocked readiness smoke with `--market-type spot --max-leverage 1` -> `readiness_gate_met=false`, `live_trading_allowed=false`, `git.dirty=true`, `generated_at_ns` present, `places_orders=false`, and blockers include `git_dirty`, missing live promotion, ADR-013 Draft, and testnet continuity evidence missing.
- Current startup-guard smoke with that fresh but blocked readiness report and `--max-readiness-report-age-seconds 86400` -> exit **2**, `startup_allowed=false`, `live_trading_authorized=false`, `git_clean=blocked`, `readiness_fresh=true`, and readiness failure includes `readiness_git_dirty`.
