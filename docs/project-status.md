...
On 2026-07-01, after adding dashboard visibility for readiness-vs-startup git evidence:

- `apps.strategies_nautilus.runners.live_startup_guard` now records the saved
  readiness report's `git` payload alongside the startup guard's expected git
  commit and clean-state requirement.
- `dashboard.snapshot.v1` startup-guard evidence now includes `Readiness git
  commit match`. A passing startup-guard report is marked blocked by the
  dashboard if the saved readiness git evidence is missing, dirty, or from a
  different commit than the startup preflight.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **62 passed**.
...
- Blocked readiness/startup/dashboard smoke with an attached testnet bundle ->
  ...
  `phase6.state=blocked`, and the dashboard's `Readiness git commit match`
  evidence row is `blocked` with dirty-readiness detail while the worktree is
