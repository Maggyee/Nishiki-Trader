...
- **Status file**: Active
- **Last updated**: 2026-08-13 (Protocol v25 all-chain/Ethereum TVL qualified; Bitcoin TVL rejected)
...
- **Current objective**: Phase 5 remains active and `stop_before_testnet_resume` remains in force. The original 16-candidate registry and Protocol v2-v5 research identities are frozen. Protocol v5 curve carry and BVOL relief are rejected and the 2026-08..12 blind remains sealed. The provider/timer review is complete: v5 and completed v2 timers are `inactive/disabled`, v5 collector status is `archived`, and a four-day post-window audit found zero new invocations. The retained v5 tree is 13 snapshots / 13 Parquets with zero conflicts; v2 `COMPLETE`, ledger, deployment identity and all recorded checksums remain unchanged. Dashboard renders the archive as a valid terminal state with no retry or operations issue. Independent Protocol v6 remains `blocked_provider_qualification` on the official 12-row grid, with historical development closed.
  ...
  Protocol v25 opened each locked DefiLlama TVL URL once from freeze `c69f579`. All-chain and Ethereum series each qualify on 1,096 unfilled development days (61 warmup, gap 1, 1,321 later rows ignored). Bitcoin-chain TVL is provider-rejected: 652 development observations versus 700 required, no snapshot/factor written, no retry. The two qualified identities may now generate 2020-2022 signals and two clean cash replays each. Confirmation, SourcePolicy, paper shadow, testnet, live trading, and the future blind remain sealed.
...
21. Preserve Protocol v25's Bitcoin-chain TVL provider reject. Do not retry that GET, write a snapshot from a second fetch, retune the 700-observation gate, or substitute another chain. All-chain and Ethereum development PnL remain unopened until two clean-git cash replays exist.
...
25. Generate 2020-2022 signals for the two Protocol v25 provider-qualified TVL identities and run exactly two clean-git Nautilus cash replays each. Do not retry Bitcoin-chain TVL, use ignored post-2022 rows, or open confirmation.
...
- No Protocol v24 reopen. All three Fear and Greed classification holds failed
  ...
  the future blind remain sealed.
- No Protocol v25 Bitcoin-chain TVL retry. All-chain/Ethereum confirmation
  PnL and the future blind remain sealed until development passers exist.
