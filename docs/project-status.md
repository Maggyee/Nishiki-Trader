...
- **Current objective**: ADR-008 Phase 3b controlled Binance Spot testnet connection probe is now landed behind every startup gate; next is the first live-network probe run + retro, while keeping `freqai_linear_v1` on `paper_simulated` and preserving the credential/order-path guards.
...
3. The Phase 3b connection probe (`testnet_runner.py --connect-probe`) is implemented and unit-tested behind every startup gate, but **no live-network probe run has been performed yet**. The first run needs real `BINANCE_TESTNET_*` env vars, clean git, and a recorded retro in `docs/retros/`.
...
2. Run the first controlled Binance Spot testnet connection probe with real `BINANCE_TESTNET_*` env vars (`python -m apps.strategies_nautilus.runners.testnet_runner --mode testnet --kind testnet --allow-real-credentials --source freqai_linear_v1 --model-version linear-mom-train20240105 --policy-position-pct-multiplier 0.2 --connect-probe --max-connect-seconds 30`). Capture the resulting `data/testnet/<run_id>/connection_probe.json` + `logs/runtime.log` outcome (connection succeeded, error if any, `node_built`, `node_run_invoked`), and record a `docs/retros/` entry. The probe must register no strategies/actors, so even a fully connected node cannot submit an order.
3. Keep the `testnet_runner.py` startup guard as the first line of defense: explicit `--allow-real-credentials`, clean git, source/model retro evidence, testnet multiplier cap, key-prefix-only audit, and Binance Spot TESTNET-only adapter config before any network connection is opened. The probe injects credentials into the in-memory `TradingNodeConfig` only and never writes the full key/secret to `logs/runtime.log` or `connection_probe.json`.
...
On 2026-05-18, after the Phase 3b connection probe was implemented behind the existing startup gates:

- `UV_CACHE_DIR=/tmp/uv-cache uv run pytest -q` -> 326 passed (7 new tests for `run_connection_probe` + CLI dispatch + no-live-credential grep guard).
