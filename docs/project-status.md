...
On 2026-07-01, after adding dashboard visibility for startup-side readiness freshness evidence:

- `dashboard.snapshot.v1` startup-guard evidence now includes `Readiness
  freshness window`, derived from the startup guard's
  `live_readiness_report.freshness` payload.
- A passing startup-guard report is now marked blocked by the dashboard if the
  startup artifact is old enough to omit readiness freshness evidence or if
  startup reported the saved readiness artifact as stale, missing
  `generated_at_ns`, generated in the future, or checked with an invalid max
  age.
...
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **60 passed**.
...
- Blocked readiness/startup/dashboard smoke with an attached testnet bundle ->
  ...
  `phase6.state=blocked`, and the dashboard's `Readiness freshness window`
  evidence row is `ok`.
