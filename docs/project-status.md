...
- **Current objective**: Phase 5 remains active and `stop_before_testnet_resume` remains in force. The original 16-candidate registry remains frozen at 16 rejects and an empty selected set. Research Protocol v2 has three locked research-only candidates. Credential-free July qualification produced independently verifiable current snapshots for Deribit BTC options (876 contracts/12 expiries) and Binance COIN-M current/next quarterly basis. The basis snapshot also passes a PnL-free point-in-time transformation with its decision delayed to the next UTC day; no signal was generated. Coin Metrics Community does not authorize USDT `TxTfrValUSD`, so the stablecoin route is `blocked_provider_entitlement` rather than substituting a different metric. Opened 2023-2025 data is diagnostic-only, 2020-2022 remains unconsumed, July 2026 has PnL forbidden, and 2026-08..12 remains the final future blind. No grid search, failed-model retuning, PnL-selected ensemble, policy change, or testnet resume is allowed.
...
2. Continue immutable daily July option/basis snapshots and require contiguous multi-day coverage before factor CSV or signal-generation qualification. Basis transformation is ready; option structural replication remains unimplemented. Keep stablecoin blocked until licensed `TxTfrValUSD` point-in-time access exists.
...
- Verification: 30 targeted tests and the full **784-test** suite passed, with
