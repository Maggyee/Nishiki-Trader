...
On 2026-07-01, after surfacing Phase 6 promotion-review fingerprint evidence in the read-only dashboard:

- `dashboard.snapshot.v1` Phase 6 report summaries now include read-only `evidence` rows for promotion-review artifact status and readiness/startup SHA-256 matching.
- The dashboard snapshot marks old or incomplete readiness artifacts as blocked when they claim `readiness_gate_met=true` but do not record promotion-review fingerprint evidence.
- `apps/frontend` now renders Phase 6 evidence rows with status, short fingerprint, and detail for attached readiness/startup guard artifacts.
- ADR-012 and `apps/frontend/README.md` now document that this is display-only evidence visibility, not report generation or live authorization.
- The change remains passive: it reads saved JSON artifacts only; it does not run `live_readiness`, run `live_startup_guard`, load credentials, build or start Nautilus, connect to Binance, write `SignalEvent`, mutate `SourcePolicy`, place orders, add a live runner, or authorize live trading.
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_dashboard_snapshot.py -q` -> **19 passed**.
...
- Dashboard snapshot smoke with attached blocked Phase 6 readiness/startup artifacts -> `phase6.state=blocked`, both reports `status=blocked`, and evidence rows include `Promotion review artifact`, `Startup promotion review artifact`, and `Readiness promotion SHA-256 match`.
- `curl -sS --max-time 10 http://127.0.0.1:3001/?lang=zh-CN` against a temporary dev server using that snapshot -> HTTP 200 content containing the Phase 6 evidence / fingerprint UI text.
