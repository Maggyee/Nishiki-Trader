...
On 2026-07-01, after adding startup/dashboard visibility for readiness-vs-startup project-status evidence:

- `apps.strategies_nautilus.runners.live_startup_guard` now accepts
  `--project-status-path` (default `docs/project-status.md`), records the
  startup-side expected project-status SHA-256, and rejects saved readiness
  reports whose recorded project-status bytes no longer match startup.
- `dashboard.snapshot.v1` startup-guard evidence now includes `Readiness
  project-status SHA-256 match`. A passing startup-guard report is marked
  blocked by the dashboard if readiness and startup project-status fingerprints
  differ or the startup-side expected SHA is missing.
- The frontend already renders Phase 6 evidence rows generically, so no
  frontend write/control surface changed.
- The change remains passive: it reads saved JSON artifacts only; it does not
  run a live runner, load credentials, build or start Nautilus, connect to
  Binance, write `SignalEvent`, mutate `SourcePolicy`, place orders, or
  authorize live trading.
- `TMPDIR=/tmp UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/ops/test_live_readiness.py tests/strategies_nautilus/test_live_startup_guard.py tests/ops/test_dashboard_snapshot.py -q` -> **64 passed**.
- `UV_CACHE_DIR=/tmp/uv-cache uv run ruff check apps tests docs infra` -> clean.
- Blocked readiness/startup/dashboard smoke with an attached testnet bundle ->
  startup guard exit **2**, `startup_allowed=false`,
  `live_trading_authorized=false`, `places_orders=false`,
  `phase6.state=blocked`; the dashboard's `Readiness project-status SHA-256
  match` row is `ok`, while `Readiness git commit match` remains `blocked`
  with dirty-readiness detail because the worktree is intentionally
  uncommitted.
