...
1. Complete the 24-hour wall-clock paper soak on home-frp with `git_dirty=false`, then write a dedicated Phase 3a retro before treating §7.2 item 4 as complete.
...
2. Finish the currently running 24-hour wall-clock soak (`data/paper/20260517-092832Z-db85d400` on home-frp) and record heartbeat cadence, `data_gap_count`, `ws_reconnect_count`, `duplicate_bars_dropped`, shutdown reason, and manifest fingerprint in a dedicated retro.
...
- Phase 3a 24h wall-clock soak is running on home-frp as `data/paper/20260517-092832Z-db85d400` with `--max-duration-seconds=86400`; it is not accepted evidence until it exits cleanly and a dedicated retro records the final manifest fingerprint and runtime counters.
