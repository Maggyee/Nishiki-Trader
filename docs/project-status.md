...
- **Status file**: Active
- **Last updated**: 2026-08-13 (Protocol v18 VXN paper-shadow collector frozen; Day 1 and approved crontab pending this push)
...
14. Preserve v17's schema mismatch, the v18 VXEEM/VXEFA rejects, and confirmed VXN at its reviewed `paper_shadow` policy. Do not retune, disable dry-run, enter `paper_simulated`, or open the future blind. Run Day 1 from the pushed collector, then install the approved weekday 03:30 UTC crontab once; do not count baseline rows toward the forward gate.
...
18. For v18 VXN relief, keep `dry_run=True` and multiplier `0.2`. After this collector commit is on `origin/main`, run one Day 1 attempt, then install the operator-approved weekday 03:30 UTC crontab once. Accumulate 7 qualified days or 50 new signals before any separate human `paper_simulated` review.
...
- Forward evidence is 0/7 days and 0/50 new signals. The VXN/BTC collector is
  frozen before network access; Day 1 and the approved 03:30 UTC crontab wait
  for this commit on `origin/main`. `paper_simulated`, future blind, testnet,
