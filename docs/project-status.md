...
- **Current objective**: Continue ADR-008 Phase 3b testnet adapter work after accepting Phase 3a's 24-hour wall-clock paper evidence, while keeping `freqai_linear_v1` on `paper_simulated` and preserving the credential/order-path gates.
...
2. Complete the rest of ADR-008 §6.2 Phase 3b: wire the guarded startup path to a real Binance testnet adapter while preserving all §4 credential redaction rules.
3. Keep the `testnet_runner.py` startup guard as the first line of defense: explicit `--allow-real-credentials`, clean git, source/model retro evidence, testnet multiplier cap, and key-prefix-only audit before any adapter is instantiated.
