# Project Status Archive — preserved before the 2026-09-07 reliability repair

This is historical context, not current operating guidance. Its obsolete next
steps and verification snapshots are retained verbatim below for audit.

- **Status file**: Active
- **Last updated**: 2026-08-28 (ADR-014 §11 class-adaptive breadth gate Accepted by operator; Protocol v53 momentum re-test frozen)
- **Current phase**: Phase 5 entry (read-only frontend + monitoring; live trading still blocked)
- **Current objective**: Phase 5 remains active and `stop_before_testnet_resume` remains in force. The original 16-candidate registry and Protocol v2-v5 research identities are frozen.
  Protocols v40 (CBOE VXN relief), v42 (CBOE VIX6M relief), v46 (Binance BTC Premium Delta relief), and v48 (Binance BTC Basis MA relief) have all completed their pre-registered 7-day forward paper shadow collection gates with zero data/schema drift and full snapshot SHA256 integrity. All four candidates are reviewed and held at `paper_shadow` under `SourcePolicy(dry_run=True, position_pct_multiplier=0.2, min_confidence_override=None)`. Live trading remains strictly blocked by the Phase 6 gate and the 2026-09..2027-01 future blind remains sealed.
  Protocol v47 (Binance BTC Realized Volatility Relief) passed 2020-2022 development (+20.77 USDT base, 22/36 months) but missed the strict 2023-2025 confirmation monthly breadth gate (16/36 vs >=18 required); Protocol v47 is closed.
  Protocol v48 (Binance Perpetual Basis Moving-Average Relief: `rule_crypto_basis_relief_v1 / crypto-btc-basis-below-ma10-lag1d-v1`) passed 2020-2022 development replays (+8.93 USDT base, 21/36 months, 2/3 years) and passed 2023-2025 confirmation holdout (+27.71 USDT base, 2/3 positive years, 20/36 positive months, 209 positions, leave-best +18.87 USDT). Candidate is held at `paper_shadow` under `SourcePolicy(dry_run=True, position_pct_multiplier=0.2, min_confidence_override=None)`. Completed 7/7 daily collection runs (93 signals stored) with 04:15 UTC daily crontab installed. The 2026-09..2027-01 future blind remains sealed.
  ...
  **ADR-014 (Research Program v2) is Accepted as of 2026-08-27.** The `external_index_relief` family (all ten current survivors) is declared saturated: protocols v49+ must pre-register an `open` mechanism family with an orthogonality rationale (registry: `docs/progress/research-mechanism-family-registry.json`, enforced by `apps/ops/research_family_registry.py --check` in the test suite) and run under Gates v2 (benchmark-relative base PnL, bootstrap p ≤ 0.10, ≥60 positions/window, deflated-expectation reporting, directional declaration, weekend-coverage statement). The first program meta-analysis (`docs/progress/research-program-meta-analysis-v1.md`) shows: buy-and-hold alone passes the historical gate set on 2023-2025 (+71.03 USDT, 23/36 months, 2/3 years); no survivor beat B&H (capture 19.3%–99.5%); random long/flat timing passes both historical stages 7.39% of the time, so 108 PnL-opened identities are expected to yield 7.98 lucky two-stage survivors vs 10 observed (P(≥10)=0.275). Survivor holds are unchanged, but per ADR-014 §6 any `paper_shadow → paper_simulated` review must confront these numbers, promotion review is portfolio-level, and per §6.3 no current survivor qualifies for standalone testnet effort. `docs/research-program-warnings.md` (W1–W10) is now Always Read; exploration continues only along `docs/research-mechanism-backlog.md` (B1 multi-factor ML on the archived point-in-time panel is priority 1). Retro: `docs/retros/2026-08-27-research-program-v2-audit-and-meta-analysis.md`.
  **Protocol v49 (B1, `freqai_panel_ridge_v1 / panel17-ridge-mwf-oos2020h2-v1`, family `multifactor_ml`) is closed: `development_rejected` under Gates v2.** The contract froze 17 archived factor snapshot pairs (SHA256-pinned) at `be1250d` before any value access; qualification passed (34/34 hashes, 2223-row official closes CSV); development (ridge monthly walk-forward, α=10 selected from the frozen grid, 2020-07..2022-12 OOS, duplicate replays reproducible) produced +11.99 USDT base and beat the exposure-matched B&H floor (+4.14) but failed leave-best (−1.06) and the bootstrap gate (p=0.2197 > 0.10). Confirmation was never opened; the identity is permanently dead — no retuning, grid extension, or ensemble. First live demonstration that Gates v2 rejects what legacy gates would arguably have passed. ML layer stays empty per ADR-014 §7. Retro: `docs/retros/2026-08-27-research-v49-panel-ridge-development.md`.
  **Protocols v50/v51 (B2, BTC funding-rate positioning states) are both closed.** v50: `blocked_provider_qualification` — its frozen 2019-12 start month does not exist in the Vision fundingRate archive (HTTP 404 on the first GET, zero values opened). v51 (the v17→v18-style recovery with `_v2` identities, corrected 2020-01 start, shared harness `apps/ops/research_funding_states.py` per ADR-014 §6.4): `development_rejected` — below-baseline (−1.10 base, p=0.60) and overheat-flat (+6.55, p=0.52, loses to its 91% exposure benchmark) carry no edge; `fund_neg_3d` (contrarian long on negative 72h funding) posted the strongest Gates-v2 numbers yet (+16.68 base, 3/3 years, leave-best +6.54, 15× its benchmark floor, p=0.096) but failed the activity floor (31<60) and monthly breadth (13/36) — structurally unreachable for a 12%-exposure episodic rule. Confirmation was never opened; all six identities are permanently dead. The retro flags the episodic-strategy blind spot in Gates v2: any breadth-gate change must be a prospective ADR-014 amendment plus a genuinely new identity, never a resurrection of v51. Retro: `docs/retros/2026-08-27-research-v50-v51-funding-states.md`.
  **Protocol v52 (B3, cross-sectional long/short states, family `cross_sectional_relative`) is pre-registered and frozen** — the program's first two-sided, multi-asset protocol. Three mechanisms under one frozen construction (30d momentum, 7d reversal, 30d low-vol; long top-3 / short bottom-3 of a 22-pair long-listed USDT pool; equal weight, 100 USDT legs, weekly Monday-close rebalance; mechanical universe/delisting rules; staged fetch boundary). Gates v2 mapped to the dollar-neutral class with a **rank-permutation null** (20k trials) replacing random timing. Contract `apps/ops/research_protocol_v52.py`; shared harness `apps/ops/research_xs_portfolio.py`; binding `apps/ops/research_v52_xs.py`. Known limitations recorded: survivorship-biased universe, no short-leg funding/borrow drag.
  **Protocol v52 is closed: `development_rejected`.** Universe qualified 22/22; 7d reversal (−739.10) and 30d low-vol (−492.89) are decisively negative after costs; `xs_mom_30d` posted the strongest Gates-v2 numbers in the program (+426.71 base on a 600-USDT gross book, stress +400.97, leave-best +226.36 over 429 legs, dollar-neutral, rank-permutation p=0.0556) but failed exactly one gate — 17/36 positive months vs ≥18, the same margin that rejected v19 VXD and v35 VIX1Y — so the rejection stands and all three identities are permanently dead; confirmation was never opened or fetched. This is the second independent gate-design observation (after v51's episodic blind spot). Retro: `docs/retros/2026-08-27-research-v52-cross-sectional-development.md`.
  **ADR-014 §11 (class-adaptive breadth gate) is Accepted as of 2026-08-28 by explicit operator directive**, and **Protocol v53 is pre-registered and frozen as its first application**: the unchanged v52 30d cross-sectional momentum rule under the new identity `rule_crypto_xs_momentum_ls_v2 / crypto-xs-mom30d-top3-bottom3-weekly-v2`, single candidate, everything else verbatim from v52 (pool, universe/delisting rules, construction, costs, windows, staged fetch boundary, permutation null). The §11 gate is operationalized in the shared harness: the rank-permutation null now also returns its positive-month median, and the candidate must exceed it (fixed 18/36 floor kept as reporting reference; pre-§11 protocols keep their frozen floors). The contract records a **conditional re-test disclosure**: v52's development numbers are known in advance, so v53's evidentiary weight rests entirely on the 2023-2025 confirmation window, which v52 never fetched. The rejected v52 reversal and low-vol mechanisms are not re-registered.
  **Protocol v53 is closed: `confirmation_rejected` on the pristine 2023-2025 window.** Development passed under §11 (17/36 months vs null median 16.0, p=0.0556, all other gates green) and opened the first-ever fetch of the confirmation archives (XMRUSDT truncated at its 2024 delisting per the frozen rule). Confirmation verdict: base +42.25 (vs +426.71 in development), stress +18.19, 18/36 months vs null median 15.0 (breadth passed — the gate was not the binding issue), 401 legs, **leave-best −226.35 (fail)**, **permutation p=0.1695 (fail)**; a single 100-USDT BTC buy-and-hold leg earned +427.47 over the same window. The development edge was regime-specific/selection luck; the sealed confirmation did its job. Identity permanently dead; the §11 gate is validated as fairer-not-looser. Gates-v2 era tally: v49–v53, 12 identities, zero survivors, zero false promotions. Retro: `docs/retros/2026-08-28-research-v53-xs-momentum-retest.md`. Natural next protocol if desired: §11 re-test of the v51 episodic negative-funding shape under a new identity (its blocking gate was breadth).
...
- `docs/progress/phase-2-research-protocol-v17.md` — Cboe VXEEM/VXEFA/VXN
  batch blocked at provider qualification by a VXEEM OHLC header mismatch.
- `docs/progress/phase-2-research-protocol-v18.md` — OHLC recovery of the
  unchanged v17 rules; confirmed VXN is held at `paper_shadow` with a frozen collector.
- `docs/progress/phase-2-research-protocol-v19.md` — RVX/VXD/VXFXI five-observation
  relief; all three rejected, VXD after confirmation monthly-breadth failure.
- `docs/progress/phase-2-research-protocol-v20.md` — OFR financial-stress relief;
  all three rejected, safe-asset after confirmation monthly-breadth failure.
- `docs/progress/phase-2-research-protocol-v22.md` — COR1M relief passed
  independent confirmation and is held at identity-specific ADR-007
...
- `docs/progress/phase-2-research-protocol-v27.md` — DefiLlama protocol-fee
  expansion; protocol closed after development cost/concentration failure.
...
- `docs/progress/phase-2-research-protocol-v30.md` — Cboe single-name vol
  relief; protocol closed after Google/Alphabet confirmation leave-best failure.
...
- `docs/progress/phase-2-research-protocol-v37.md` — Cboe cross-asset equity buywrite & tech overwrite expansion
  (BXN, BXY, BXR); protocol closed after BXN confirmation monthly breadth and leave-best failure.
- `docs/progress/phase-2-research-protocol-v38.md` — Cboe hedged equity & daily VRP harvesting expansion
...
- `docs/progress/phase-2-research-protocol-v40.md` — Cboe cross-asset equity implied volatility relief expansion
  (VXN, RVX, VXD); VXN passed 2023-2025 confirmation, completed 7/7 paper_shadow forward collection days (20 signals stored), held at `paper_shadow`.
...
- `docs/progress/phase-2-research-protocol-v42.md` — Cboe volatility term structure & horizon relief expansion
  (VIX9D, VIX3M, VIX6M); VIX6M passed 2023-2025 confirmation, completed 7/7 paper_shadow forward collection days (25 signals stored), held at `paper_shadow`.
...
- `docs/progress/phase-2-research-protocol-v46.md` — Binance Perpetual Premium Index standard basis relief
  (BTC Premium 5-day delta < 0.0 standard relief); passed 2023-2025 confirmation (+13.68 USDT base, 2/3 positive years, 21/36 positive months, leave-best +7.22 USDT), completed 8/7 paper_shadow forward runs (99 signals stored), held at `paper_shadow`.
- `docs/progress/phase-2-research-portfolio-shadow-report.md` — Phase 5 multi-candidate forward shadow portfolio monitor report (10 candidates, cross-correlation matrix, joint exposure profile).
...
22. Preserve all three Protocol v26 rejects. Do not retry Solana DEX volume, retune/sign-flip/ensemble all-DEX or Ethereum volume expansion, reopen 2023-2025 DEX-volume PnL, or claim historical vintages.
23. Preserve all three Protocol v27 rejects. Do not retune/sign-flip/ensemble fees, revenue, or holder-revenue expansion, reopen 2023-2025 fee PnL, or claim historical vintages.
24. Preserve all three Protocol v28 provider rejects. Do not retry options notional, options premium, or perpetual OI, loosen the 700-observation gate, or open signals/PnL under this identity.
25. Operate under ADR-014: read `docs/research-program-warnings.md` before any research/gating/promotion task; do not propose new `external_index_relief` members; keep `apps/ops/research_family_registry.py --check` green (update the family table in the same commit that adds any `research_protocol_v*.py`); apply Gates v2 to every new pre-registration; treat `docs/progress/research-program-meta-analysis-v1.md` as the current multiplicity calibration.
...
22. Keep v22 COR1M relief at the reviewed `paper_shadow` policy. The next
    implementation step is a fail-closed prospective COR1M/BTC collector and
...
29. Keep Protocol v30 closed. Any later independent single-name or sector vol study needs a new identity frozen before data access; it may not continue these three fingerprints, retune VXGOG after seeing the leave-best miss, or open the future blind.
...
34. Pick the next alpha study from `docs/research-mechanism-backlog.md` in priority order (B1 multi-factor ML on the archived point-in-time panel first), pre-registered under ADR-014 §3.4 with Gates v2. Any `paper_shadow → paper_simulated` review must include the ADR-014 §6 portfolio marginal-contribution evidence and the meta-analysis lucky-survivor expectation.
...
- No Protocol v26 reopen. Solana is provider-rejected and all-DEX/Ethereum
  volume expansion failed development; confirmation, sign flip, ensemble, and
  the future blind remain sealed.
- No Protocol v27 reopen. All three fee/revenue identities failed
  development; confirmation, sign flip, ensemble, and the future blind remain
  sealed.
...
- No Protocol v30 reopen. Apple and Amazon failed development; Google/Alphabet
  vol relief failed confirmation leave-best. Retry, sign flip, ensemble,
  mega-cap substitution after seeing the result, and the future blind remain
  sealed.

- No Protocol v31 reopen. VXSLV and VXXLE failed coverage; VXTLT failed
...
- EUVIX and JYVIX failed the locked last-observation gate after one GET each.
  No snapshot, signal, or PnL was written for those kinds.
- BPVIX qualified on 754 unfilled 2020-2022 observations. Factors were written
  from the one GET. Signals and development PnL remain unopened.

On 2026-08-13, after Protocol v32 pre-registration:

- Official Cboe historical-data and EUVIX/JYVIX/BPVIX identity pages were
  inspected. No historical CSV body, close value, signal, or PnL was opened.
- The contract locks Euro, Yen, and Pound FX five-observation implied-vol
  relief, a one-calendar-day lag, one GET per index, unused post-2022 rows,
  the v7 downtime catalog, and no historical-vintage claim.

On 2026-08-13, after Protocol v31 development:

- Long-Treasury ETF vol relief is rejected on 2020-2022: base/stress
  -7.758932/-9.327215 USDT, 1/3 years, and negative leave-best. Duplicate
  fills from `d5eeb39` match.
- Protocol v31 is closed. Confirmation remains sealed. No identity is
  eligible for ADR-007.

On 2026-08-13, after Protocol v31 provider qualification:

- VXSLV and VXXLE each have 533 development observations versus 700 required.
  No snapshot, signal, or PnL was written for those kinds.
- VXTLT qualified on 755 unfilled 2020-2022 observations. Factors were written
  from the one GET. Signals and development PnL remain unopened.

On 2026-08-13, after Protocol v31 pre-registration:

- Official Cboe historical-data and VXSLV/VXXLE/VXTLT identity pages were
  inspected. No historical CSV body, close value, signal, or PnL was opened.
- The contract locks silver, energy-sector, and long-Treasury ETF
  five-observation implied-vol relief, a one-calendar-day lag, one GET per
  index, unused post-2022 rows, the v7 downtime catalog, and no
  historical-vintage claim.

On 2026-08-13, after Protocol v30 confirmation review:

- Duplicate cash replays from `69fa1a9` reject unchanged Google/Alphabet vol
  relief solely on leave-best concentration. Protocol v30 is closed.

On 2026-08-13, after Protocol v30 development review:

- Duplicate cash replays from `b2c85b2` reject Apple and Amazon and pass
  unchanged Google/Alphabet vol relief. Confirmation remains sealed.

On 2026-08-13, after Protocol v29 development review:

- Duplicate cash replays from `a19c207` reject all three identities. Recession
  attention relief is cost-positive but has only one positive year. Fed and
  Inflation fail after costs. Confirmation remains sealed.

On 2026-08-13, after Protocol v28 provider qualification:

- Options notional and premium each have 414 development observations versus
  700 required. Open interest has 675. No snapshot or factor was written.
- Protocol v28 is closed at provider qualification. Confirmation remains sealed.

On 2026-08-13, after Protocol v28 pre-registration:

- Official DefiLlama free-API documentation was inspected; no historical
  options-volume or open-interest JSON body, value, signal, or PnL was opened.
- The contract locks options notional, options premium, and perpetual OI
  five-observation expansion, a two-calendar-day lag, one GET per kind, unused
  post-2022 rows, the v7 downtime catalog, and no historical-vintage claim.

On 2026-08-13, after Protocol v27 development:

- All-chain fees expansion is rejected on leave-best concentration despite
  positive base/stress and 19/36 months. Duplicate fills from `66a725e` match.
- Revenue and holder-revenue fail cost gates; holder-revenue also has 14/36
  positive months.
- Protocol v27 is closed. Confirmation remains sealed. No identity is
  eligible for ADR-007.

On 2026-08-13, after Protocol v27 provider qualification:

- All-chain dailyFees, dailyRevenue, and dailyHoldersRevenue each have 1,096
  unfilled development observations, 61 warmup rows, a one-day maximum gap,
  and 1,321 ignored later rows. Factors were written from the one GET per
  dataType.
- Signals and development PnL remain unopened.

On 2026-08-13, after Protocol v27 pre-registration:

- Official DefiLlama free-API documentation was inspected; no historical fee
  JSON body, value, signal, or PnL was opened.
- The contract locks all-chain dailyFees, dailyRevenue, and dailyHoldersRevenue
  five-observation expansion, a two-calendar-day lag, one GET per dataType,
  unused post-2022 rows, the v7 downtime catalog, and no historical-vintage
  claim.

On 2026-08-13, after Protocol v26 development:

- All-DEX volume expansion is rejected on 2020-2022: base/stress
  -6.320471/-9.854272 USDT and negative leave-best. Duplicate fills from
  `8abaffb` match.
- Ethereum DEX volume expansion is rejected on the same three cost gates:
  base/stress -2.029904/-5.636548 USDT.
- Protocol v26 is closed. Confirmation remains sealed. No identity is
  eligible for ADR-007.
...
- Official DefiLlama free-API documentation was inspected; no historical DEX
  volume JSON body, value, signal, or PnL was opened.
- The contract locks all-DEX, Ethereum, and Solana five-observation volume
  expansion, a two-calendar-day lag, one GET per kind, unused post-2022 rows,
  the v7 downtime catalog, and no historical-vintage claim.

On 2026-08-13, after Protocol v25 confirmation:

- All-chain TVL expansion is rejected on 2023-2025: 14/36 positive months and
  negative leave-best base PnL. Duplicate fills from `0bb16d3` match.
- Protocol v25 is closed. No identity is eligible for ADR-007.
...
- All-chain TVL expansion passes frozen 2020-2022 gates on duplicate clean-git
  cash replays from `e4b22c0`.
- Ethereum TVL expansion is rejected on monthly breadth and leave-best
  concentration. Bitcoin-chain TVL remains provider-rejected.
- Confirmation values and PnL remain sealed.

On 2026-08-13, after Protocol v25 provider qualification:

- All-chain and Ethereum TVL each have 1,096 unfilled development
  observations, 61 warmup rows, a one-day maximum gap, and 1,321 ignored
  later rows. Factors were written from the one GET per kind.
- Bitcoin-chain TVL failed closed after its one GET: 652 development
  observations. No snapshot, factor, signal, or PnL was written.
- Signals and development PnL for the two qualified identities remain unopened.

On 2026-08-13, after Protocol v25 pre-registration:

- Official DefiLlama free-API documentation was inspected; no historical TVL
  JSON body, value, signal, or PnL was opened.
- The contract locks all-chain, Ethereum, and Bitcoin five-observation TVL
  expansion, a two-calendar-day lag, one GET per kind, unused post-2022 rows,
  the v7 downtime catalog, and no historical-vintage claim.

On 2026-08-13, after Protocol v24 development:

- Six bundles from clean commit `7127677` reproduce pairwise with identical
  fills, zero shorts, blockers, or verified no-kline event hits.
- Extreme Fear hold base/stress is -9.876116/-10.883138 USDT (1/3 years,
  8/36 months). Fear hold is -28.166744/-28.646468 USDT. Non-greed hold is
  -29.504746/-29.895812 USDT with 24 positions.
- All three are rejected. Confirmation remains sealed.

On 2026-08-13, after Protocol v24 provider qualification:

- The one locked GET from freeze `1b5ff65` produced an immutable snapshot that
  re-verifies at `sha256:8488ecdf…2d5cc7bf`.
- Development has 1,096 unfilled rows, 61 warmup rows, a one-day maximum gap,
  and 1,319 later rows counted but unused. Factor SHA-256 is `b7c66c31…`.
- No signal, return, PnL, confirmation use, policy, trading path, or future
  blind was opened during qualification.
...
- Six bundles from clean commit `0b8c517` reproduce pairwise with identical
  fills, zero shorts, blockers, or verified no-kline event hits.
- Bitcoin attention base/stress is -38.528123/-41.147921 USDT (1/3 years,
  17/36 months). Cryptocurrency attention is -25.213814/-27.703870 USDT.
  Ethereum attention is gross-positive at +8.830440 USDT but base/stress
  -0.754934/-3.151278 USDT with leave-best -15.168838 USDT.
- All three are rejected. Confirmation remains sealed.
...
- Official Wikimedia pageview API documentation was inspected; no
  `/metrics/pageviews` JSON body, view count, signal, or PnL was opened.
- The contract locks Bitcoin, Ethereum, and Cryptocurrency user pageviews,
  five-observation strictly-positive expansion, a two-calendar-day lag, one
  GET per article through 2022-12-31, the v7 downtime catalog, and no
  historical-vintage claim.
- Targeted freeze tests cover protocol identity, dry-run request plans, JSON
  audit/factor round-trip, confirmation-timestamp rejection, and buy/flat
  signal emission.
...
- The official descriptive pages, frontend series keys, and response headers
  were inspected; neither the JSON nor CSV data body was opened.
- The contract locks OFRFSI, Credit, and Flight_to_Safety under D+5 availability,
  three fixed five-observation negative-change rules, one body GET, and no
  historical-vintage claim.
...
On 2026-08-13, after Protocol v20 development:

- Six bundles from clean commit `b875e6c` reproduce pairwise with identical
  fills, zero shorts, blockers, or verified no-kline event hits.
- Total/credit stress relief base PnL is -10.913104/-33.280100 USDT; both are
  rejected.
- Safe-asset-stress relief passes at base/stress +11.916318/+10.618271 USDT,
  2/3 years, 20/36 months, 76 positions, and +0.482873 leave-best base PnL.
  Confirmation remains sealed pending a separate contract.

On 2026-08-13, after Protocol v20 confirmation pre-registration:

- The safe-asset-only contract fingerprints committed review `0ab1e08`, its
  result hash, the exact OFR snapshot, unchanged rule, D+5 decision partition,
  costs/gates, and execution audit while retaining no historical-vintage claim.
- Eleven targeted tests pass. Dry runs open no holdout value, network request,
  PnL, policy, trading path, or future blind.

On 2026-08-13, after Protocol v20 confirmation data qualification:

- D+5 `available_at` partitioning qualifies 783 decisions, 261 per year,
  exactly from 2023-01-01 through 2025-12-31, with a three-day maximum gap
  and 40 warmup rows.
- Factor SHA-256 is `0c258762…826a2e`; no network request, fill, return, PnL,
  historical-vintage claim, policy, trading path, or future blind was opened.

On 2026-08-13, after Protocol v22 pre-registration:

- Three option-surface identities, exact signs/lookback/D+1 lag, official
  Cboe URLs/schemas, costs, gates, and partitions are frozen before historical
  CSV bodies or PnL.
- Twelve targeted contract, synthetic scalar/OHLC snapshot, and signal tests
  pass. All three request plans remain offline dry-runs.

On 2026-08-13, after Protocol v22 provider qualification:

- SKEW/COR1M each qualify on 756 development rows plus 41 warmup rows, with a
  four-day maximum gap and no fill/interpolation.
- DSPX fails the frozen positive-value contract and is provider-rejected
  without retry, snapshot, factor, signal, or PnL. Development PnL remains
  sealed for the two qualified candidates.

On 2026-08-13, after the Protocol v22 execution correction:

- Four processes failed before engine startup because the confirmation-only
  catalog has no 2020-2022 bars; zero bundle/order/fill/PnL was produced.
- A separate amendment freezes the audited 2020-2022 development catalog and
  leaves all research identities, evidence gates, confirmation, policy, and
  trading boundaries unchanged.

On 2026-08-13, after Protocol v22 development:

- Two clean replays per qualified candidate reproduce with identical fills,
  no shorts, effective blockers, or verified no-kline event hits.
- SKEW relief is rejected. COR1M relief passes at base/stress
  +14.196850/+12.756155 USDT, 2/3 years, 23/36 months, 86 positions, and
  +0.856160 leave-best base PnL; confirmation remains sealed.

On 2026-08-13, after Protocol v22 confirmation pre-registration:

- The COR1M-only contract fingerprints committed development evidence, exact
  snapshot/rule/D+1 partition, current-history limitation, execution audit,
  costs, gates, and two replays before holdout values.
- Targeted dry runs open no network request, holdout value, PnL, policy,
  trading path, existing collector, or future blind.

On 2026-08-13, after Protocol v22 confirmation data qualification:

- Offline extraction qualifies 752 COR1M observations in 2023-2025, 42
...
On 2026-08-13, after the identity-specific Protocol v22 ADR-007 review:

- `rule_cboe_implied_correlation_relief_v1 /
  cboe-cor1m-diff5-negative-lag1d-v1` is held at `paper_shadow` under
  `SourcePolicy(dry_run=True, position_pct_multiplier=0.2,
  min_confidence_override=None)`; the policy review is allowed with no review
  blockers.
- Clean bundle `data/research-v22/paper/20260813-045912Z-903c37c0` accepts
  141/141 historical SignalEvent v1 rows on the longest contiguous catalog
  suffix; all 141 are dry-run, with 0 skips, orders, fills, positions,
  unauthorized/expired/lag/data-gap signals, runtime gaps, or kill-switches.
- The 1,014-day historical replay is entry-plumbing evidence only. Forward
  progress is 0/7 qualified days and 0/50 new signals, so `paper_simulated`
  remains unauthorized. No collector/scheduler, credential, testnet/live path,
  or future blind was opened. The next action is to implement and review a
  fail-closed prospective collector before any schedule installation.

On 2026-08-13, after Protocol v21 pre-registration:

- One net-liquidity identity, exact three-series formula, four-observation
  change, positive sign, D+7 lag, costs, gates, and partitions are frozen
...
- Two clean-git cash replays from `b99962c` reproduce with identical fills.
  Gross/base/stress PnL is +47.630810/+35.946934/+33.025964 USDT, 3/3 years,
  74 positions, and +18.107091 leave-best base PnL.
- Monthly breadth is 17/36 versus 18 required. Classification is
  `reject_candidate`. No ADR-007, `paper_shadow`, retune, or future-blind
  opening is authorized.

On 2026-08-13, after Protocol v19 development:

- Four bundles from clean commit `4f4cbee` reproduce pairwise with identical
  fills, zero shorts, blockers, or verified no-kline event hits.
- RVX is rejected at base/stress -7.025680/-8.467275 USDT; VXFXI remains a
  provider rejection without a backtest.
- Dow-volatility relief passes development at base/stress
  +17.483702/+15.887517 USDT, 2/3 years, 20/36 months, 87 positions, and
  +7.660115 leave-best base PnL. Confirmation remains sealed until a separate
  contract is frozen.

On 2026-08-13, after Protocol v19 confirmation pre-registration:

- The VXD-only contract fingerprints committed review `128aa23`, its result
  SHA-256, the exact qualified VXD snapshot, unchanged five-observation rule,
...
On 2026-08-13, after Protocol v19 VXD confirmation:

- Two clean replays from `6f342e9` have identical fills, 187 signals, 94
  long-only positions, and zero blockers or verified no-kline event hits.
- Gross/base/stress PnL is +71.365380/+57.492785/+54.024636 USDT, 3/3 years
  are positive, and leave-best is +34.350455 USDT.
- Monthly breadth is only 17/36 versus 18 required. Classification is
  `reject_candidate`; no ADR-007, paper shadow, policy change, or blind opening.

On 2026-08-13, after Protocol v19 provider qualification:

- The freeze was on `origin/main` at `a2ea80a` before access. Three HEADs
  returned 200/text-csv, followed by exactly one GET each. RVX/VXD qualified
  at 755/758 reserve rows; VXFXI failed closed at 533/700 with no retry.
- Qualified RVX/VXD snapshots re-verify; no factor, signal, PnL, confirmation,
  or future-blind row was opened during provider qualification.
- v8/v16/v18 paper shadow, SourcePolicy, testnet, and the future blind are
  unchanged.

On 2026-08-13, after Protocol v18 development:

- The separate ADR-007 review is `hold @ paper_shadow` and allowed. Its clean
  bundle accepts/dry-runs 129/129 signals with zero orders, fills, gaps, lag,
  kill-switch, or review blockers under multiplier 0.2.
- Forward evidence is 1/7 days and 0/50 new signals. Day 1 qualified from
  `e90799e` with a one-day-old VXN close, 167 closed BTC hours, 41 baseline
  signals, and zero blockers. The weekday 03:30 UTC crontab is installed once.
  `paper_simulated`, future blind, testnet, and live remain closed.
- Two confirmation replays from clean pushed commit `9360eb3` have identical
  fills. Base/stress PnL is +60.207790/+57.571400 USDT, 3/3 years and 20/36
  months are positive, 72 positions close, and leave-best is +35.304076.
- Confirmation classification is `paper_shadow_review_eligible`; no policy or
  trading path changed, and the future blind remains sealed.
- Confirmation extraction from pushed commit `d22081e` qualified 752 unfilled
  VXN observations, a four-day maximum gap, 42 warmup rows, and 144 unchanged
  signals without a network request or PnL calculation.
- The VXN-only confirmation contract now locks the committed development
  result `98a0ef2`, exact snapshot fingerprint, one-day lag, unchanged rule,
  costs/gates, and audited v8 execution catalog before holdout value access.
- Six clean-commit Nautilus replays from `a4ff746` used the frozen VXN/VXEEM/VXEFA
  rules and the already-qualified OHLC snapshots.
- VXEEM/VXEFA base PnL is -3.849353/-2.377915 USDT and both are rejected.
- Nasdaq vol relief base/stress is +14.128562/+12.631995 USDT, 2/3 years,
  21/36 months, 81 positions, leave-best +3.151905; duplicate fills match.
- Confirmation, future blind, SourcePolicy, testnet, and live paths stay closed.

On 2026-08-13, after Protocol v18 provider qualification:

- v18 was pushed at `77afac1` before the three OHLC bodies were opened.
- VXEEM/VXEFA/VXN snapshots verify with 755/755/758 unfilled reserve rows,
  no forward fill, and independent `--verify` matches.
- No factor export, SignalEvent, Nautilus PnL, confirmation, or trading path
  was opened. v17 remains closed.

On 2026-08-13, after Protocol v17 provider qualification and v18 pre-registration:
...
- Collector contract/code was pushed at `8bb03fd` before the first request;
  attempt `20260812T104309Z-a3a1f27bf08b` ran clean and is on `origin/main`.
- The D+2 Treasury snapshot has 401 unfilled observations through 2026-08-10,
  maximum four-day calendar gap and zero revisions; independent snapshot
  verification passes.
- All 167 closed BTCUSDT hourly bars are contiguous with zero revisions. Day 1
  qualified with zero blockers; 92 baseline signals seed state but forward
  progress is 1/7 days and 0/50 new signals.
- No credentials, orders, fills, policy mutation, runtime, scheduler, testnet,
  live path, or future blind was touched.
- Full verification: **1,127 passed, 12 Postgres-dependent skips**; Ruff and
