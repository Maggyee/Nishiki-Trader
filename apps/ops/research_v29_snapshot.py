"""Collect immutable official Wikimedia pageview snapshots for Protocol v29."""
...
from apps.ops.research_protocol_v29 import PROVIDER_CONTRACT, load_and_validate

SCHEMA_VERSION = "research.raw_snapshot.v29"
KINDS = ("federal_reserve", "inflation", "recession")
...
def _contract() -> dict[str, Any]:
    ...
    if payload.get("schema_version") != "research.data_sources.v29":
        raise ValueError("Protocol v29 provider schema drifted")
    return payload


def _request(kind: str) -> dict[str, Any]:
    if kind not in KINDS:
        raise ValueError(f"unsupported Protocol v29 kind {kind!r}")
...
def request_plan(kind: str) -> dict[str, Any]:
    spec = _request(kind)
    return {
        "schema_version": "research.snapshot.request_plan.v29",
        "kind": kind,
        "url": spec["url"],
...
def _reject_constant(value: str) -> None:
    raise ValueError(f"Wikimedia JSON contains non-finite constant {value!r}")


def _parse_timestamp(value: Any, *, kind: str, index: int) -> date:
    if not isinstance(value, str) or len(value) != 10 or not value.isdigit():
        raise ValueError(f"{kind} items[{index}] timestamp must be YYYYMMDDHH")
    if value[8:] != "00":
        raise ValueError(f"{kind} items[{index}] timestamp hour must be 00")
    try:
        observed = datetime.strptime(value[:8], "%Y%m%d").date()
    except ValueError as exc:
        raise ValueError(f"{kind} items[{index}] timestamp is invalid") from exc
    return observed


def _views(value: Any, *, kind: str, index: int) -> float:
    if isinstance(value, bool) or not isinstance(value, int):
        raise ValueError(f"{kind} items[{index}] views must be a non-boolean integer")
    result = float(value)
    if not math.isfinite(result) or result < 0.0:
        raise ValueError(f"{kind} items[{index}] views must be a finite non-negative integer")
    return result
...
def parse_and_audit_json(kind: str, raw: bytes) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    try:
        ...
        raise ValueError("Wikimedia pageview response must be UTF-8 JSON") from exc
    if not isinstance(payload, dict):
        raise ValueError("Wikimedia pageview JSON root must be an object")
    ...
    for index, item in enumerate(items):
        if not isinstance(item, dict) or any(field not in item for field in REQUIRED_ITEM_FIELDS):
            raise ValueError(f"{kind} items[{index}] is missing locked fields")
        if (
            item.get("project") != spec["project"]
            or item.get("article") != spec["article"]
            or item.get("granularity") != spec["granularity"]
            or item.get("access") != spec["access"]
            or item.get("agent") != spec["agent"]
        ):
            raise ValueError(f"{kind} items[{index}] identity drifted")
        observed = _parse_timestamp(item.get("timestamp"), kind=kind, index=index)
        if observed > DEVELOPMENT_END:
            raise ValueError(f"{kind} items[{index}] opened confirmation")
        if previous is not None and observed <= previous:
            raise ValueError(f"{kind} timestamps must be unique and increasing")
        previous = observed
        rows.append(
            {
                "date": observed.isoformat(),
                "value": _views(item.get("views"), kind=kind, index=index),
            }
        )
    warmup = [
    ...
    *,
    ...
    envelope = {
        **core,
        ...
        "vintage_id": f"wikimedia-{kind}:{retrieved.isoformat()}:{digest[:12]}",
    }
...
def verify_snapshot(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("invalid Protocol v29 snapshot schema")
    kind = str(envelope.get("kind"))
    spec = _request(kind)
    if envelope.get("url") != spec["url"] or envelope.get("article") != spec["article"]:
        raise ValueError("Protocol v29 snapshot request identity drifted")
    validation = load_and_validate()
    if envelope.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v29 protocol fingerprint mismatch")
    if envelope.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v29 provider fingerprint mismatch")
    if envelope.get("boundaries") != _boundaries():
        raise ValueError("Protocol v29 snapshot boundaries are unsafe")
    ...
    if envelope.get("payload_sha256") != "sha256:" + hashlib.sha256(raw).hexdigest():
        raise ValueError("Protocol v29 raw payload fingerprint mismatch")
    _, audit = parse_and_audit_json(kind, raw)
    if envelope.get("audit") != audit:
        raise ValueError("Protocol v29 snapshot audit mismatch")
    core = {
        key: value
    ...
    if envelope.get("snapshot_sha256") != "sha256:" + digest or not str(
        ...
        raise ValueError("Protocol v29 snapshot fingerprint mismatch")
    return {
        "path": str(path),
        "kind": kind,
            ...
            writer.writerow(
                {
                    ...
                    "vintage_id": envelope["vintage_id"],
                    "snapshot_sha256": envelope["snapshot_sha256"],
                    "pageviews": format(float(row["value"]), ".12g"),
                }
            )
    return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/research-v29/raw"))
