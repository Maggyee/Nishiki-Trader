"""Apply frozen Protocol v21 gates to duplicate development bundles."""
...
from apps.ops.research_protocol_v21 import IDENTITIES, load_and_validate
from apps.ops.research_v13_review import build_standard_review

SCHEMA_VERSION = "research.v21.development_results.v1"
PROVIDER_QUALIFICATION = Path("docs/progress/phase-2-research-v21-provider-qualification.json")
...
def load_provider_qualification(
    ...
    if payload.get("schema_version") != "research.v21.provider_qualification.v1":
        raise ValueError("Protocol v21 provider qualification schema drifted")
    validation = load_and_validate()
    if payload.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v21 provider qualification fingerprint drifted")
    if payload.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v21 provider contract fingerprint drifted")
    if payload.get("classification") != "provider_qualified":
        raise ValueError("Protocol v21 provider qualification status drifted")
    audit = payload.get("audit", {})
    if (
        int(audit.get("development_common_row_count", 0)) < 150
        or int(audit.get("warmup_common_row_count", 0)) < 8
        or audit.get("forward_fill_used") is not False
        or audit.get("interpolation_used") is not False
    ):
        raise ValueError("Protocol v21 provider coverage evidence drifted")
    boundaries = payload.get("boundaries", {})
    if (
        boundaries.get("factor_values_reported") is not False
        or boundaries.get("signals_generated") is not False
        or boundaries.get("pnl_opened") is not False
    ):
        raise ValueError("Protocol v21 provider qualification crossed boundaries")
    return payload
...
def build_review(
    ...
    *,
    ...
    result["provider_qualification"] = {
        ...
        "development_common_row_count": qualification["audit"]["development_common_row_count"],
