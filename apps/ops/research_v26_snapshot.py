"""Collect immutable official DefiLlama DEX volume snapshots for Protocol v26."""
...
from apps.ops.research_protocol_v26 import PROVIDER_CONTRACT, load_and_validate

SCHEMA_VERSION = "research.raw_snapshot.v26"
KINDS = ("all_dexs", "ethereum", "solana")
...
def _contract() -> dict[str, Any]:
    ...
    if payload.get("schema_version") != "research.data_sources.v26":
        raise ValueError("Protocol v26 provider schema drifted")
    return payload


def _request(kind: str) -> dict[str, Any]:
    if kind not in KINDS:
        raise ValueError(f"unsupported Protocol v26 kind {kind!r}")
...
def request_plan(kind: str) -> dict[str, Any]:
    spec = _request(kind)
    return {
        "schema_version": "research.snapshot.request_plan.v26",
        "kind": kind,
        "url": spec["url"],
...
def _reject_constant(value: str) -> None:
    raise ValueError(f"DefiLlama DEX JSON contains non-finite constant {value!r}")
...
def _volume(value: Any, *, kind: str, index: int) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValueError(f"{kind} items[{index}] volume must be numeric")
    result = float(value)
    if not math.isfinite(result) or result < 0.0:
        raise ValueError(f"{kind} items[{index}] volume must be finite and non-negative")
    return result
...
def parse_all_json_rows(kind: str, raw: bytes) -> list[dict[str, Any]]:
    try:
        ...
        raise ValueError("DefiLlama DEX response must be UTF-8 JSON") from exc
    ...
    for index, item in enumerate(chart):
        if not isinstance(item, list) or len(item) != 2:
            raise ValueError(f"{kind} items[{index}] must be a [date, volume] pair")
        observed = _parse_date(item[0], kind=kind, index=index)
        parsed.append(
            {
                "date": observed.isoformat(),
                "value": _volume(item[1], kind=kind, index=index),
            }
        )
...
def verify_snapshot(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("invalid Protocol v26 snapshot schema")
    kind = str(envelope.get("kind"))
    spec = _request(kind)
    if envelope.get("url") != spec["url"]:
        raise ValueError("Protocol v26 snapshot request identity drifted")
    validation = load_and_validate()
    if envelope.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v26 protocol fingerprint mismatch")
    if envelope.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v26 provider fingerprint mismatch")
    if envelope.get("boundaries") != _boundaries():
        raise ValueError("Protocol v26 snapshot boundaries are unsafe")
    ...
    if envelope.get("payload_sha256") != "sha256:" + hashlib.sha256(raw).hexdigest():
        raise ValueError("Protocol v26 raw payload fingerprint mismatch")
    _, audit = parse_and_audit_json(kind, raw)
    if envelope.get("audit") != audit:
        raise ValueError("Protocol v26 snapshot audit mismatch")
    core = {
        key: value
    ...
    if envelope.get("snapshot_sha256") != "sha256:" + digest or not str(
        ...
        raise ValueError("Protocol v26 snapshot fingerprint mismatch")
    return {
        "path": str(path),
        "kind": kind,
            ...
            writer.writerow(
                {
                    ...
                    "vintage_id": envelope["vintage_id"],
                    "snapshot_sha256": envelope["snapshot_sha256"],
                    "volume": format(float(row["value"]), ".12g"),
                }
            )
    return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/research-v26/raw"))
