"""Validate the frozen Protocol v35 confirmation contract."""
...
DEFAULT_CONTRACT = Path("docs/progress/phase-2-research-v35-confirmation.json")
DEVELOPMENT_RESULTS = Path("docs/progress/phase-2-research-v35-development-results.json")
SCHEMA_VERSION = "research.protocol.v35.confirmation.v1"
...
def validate_contract(payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("Protocol v35 confirmation schema drifted")
    if payload.get("status") != "preregistered_before_confirmation_open":
        raise ValueError("Protocol v35 confirmation status drifted")
    parent_validation = load_and_validate_parent()
    if payload.get("parent_protocol_sha256") != parent_validation["protocol_sha256"]:
        raise ValueError("Protocol v35 confirmation parent protocol fingerprint mismatch")
    dev_payload = json.loads(DEVELOPMENT_RESULTS.read_text())
    if dev_payload.get("schema_version") != "research.v35.development_results.v1":
        raise ValueError("Protocol v35 dev results schema drifted")
    candidate = payload.get("candidate", {})
    if candidate.get("key") != "vix1y_relief":
        raise ValueError("candidate drifted; must be vix1y_relief")
    source, model = IDENTITIES["vix1y_relief"]
