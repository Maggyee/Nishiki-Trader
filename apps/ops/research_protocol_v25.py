"""Validate frozen Protocol v25 DefiLlama TVL research contracts."""
...
DEFAULT_PROTOCOL = Path("docs/progress/phase-2-research-protocol-v25.json")
PROVIDER_CONTRACT = Path("docs/progress/phase-2-research-v25-data-sources.json")
SCHEMA_VERSION = "research.protocol.v25"
IDENTITIES = {
...
KIND_BY_CANDIDATE = {
    "all_chains_tvl_expansion": "all_chains",
    "ethereum_tvl_expansion": "ethereum",
    "bitcoin_tvl_expansion": "bitcoin",
}
COMMON_PARAMETERS = {
...
def validate_protocol(payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("schema_version") != SCHEMA_VERSION or payload.get("status") != (
        "preregistered_before_defillama_tvl_body_access"
    ):
        raise ValueError("Protocol v25 pre-access identity drifted")
    ...
    if disclosure.get("historical_json_bodies_opened") is not False or any(
        ...
        raise ValueError("Protocol v25 historical data boundary drifted")
    data = payload.get("data_contract", {})
    if data.get("provider_contract") != str(PROVIDER_CONTRACT):
        raise ValueError("Protocol v25 provider path drifted")
    if data.get("conservative_decision_lag_calendar_days") != 2:
        raise ValueError("Protocol v25 decision lag drifted")
    if data.get("historical_vintage_claim") is not False:
        raise ValueError("Protocol v25 cannot claim historical vintages")
    candidates = payload.get("candidates")
    if not isinstance(candidates, list) or len(candidates) != 3:
        raise ValueError("Protocol v25 must lock exactly three candidates")
    actual = {row["key"]: (row["source"], row["model_version"]) for row in candidates}
    if actual != IDENTITIES:
        raise ValueError("Protocol v25 candidate identities drifted")
    for row in candidates:
        key = row["key"]
        if row.get("kind") != KIND_BY_CANDIDATE[key] or row.get("parameters") != COMMON_PARAMETERS:
            raise ValueError("Protocol v25 candidate parameters drifted")
    execution = payload.get("execution", {})
    if {key: execution.get(key) for key in LOCKED_EXECUTION} != LOCKED_EXECUTION:
        raise ValueError("Protocol v25 execution contract drifted")
    if payload.get("gates", {}).get("development") != DEVELOPMENT_GATES:
        raise ValueError("Protocol v25 development gates drifted")
    ...
    if multiple.get("candidate_count_locked") != 3 or not all(
        ...
        raise ValueError("Protocol v25 multiple-testing guard drifted")
    if any(payload.get("boundaries_effect", {}).values()):
        raise ValueError("Protocol v25 boundaries are unsafe")
...
def validate_provider_contract(payload: dict[str, Any]) -> dict[str, Any]:
    if (
        payload.get("schema_version") != "research.data_sources.v25"
        or payload.get("status") != "locked_before_defillama_tvl_body_access"
    ):
        raise ValueError("Protocol v25 provider contract drifted")
    ...
    if not isinstance(requests, list) or [row.get("kind") for row in requests] != [
        ...
        raise ValueError("Protocol v25 provider request order drifted")
    ...
    if (
        ...
        raise ValueError("Protocol v25 request budget drifted")
    if payload.get("historical_vintage_claim") is not False:
        raise ValueError("Protocol v25 provider cannot claim historical vintages")
    canonical = json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False)
    return {
...
def load_and_validate(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Protocol v25 root must be an object")
    result = validate_protocol(payload)
    provider = json.loads(PROVIDER_CONTRACT.read_text())
    if not isinstance(provider, dict):
        raise ValueError("Protocol v25 provider root must be an object")
