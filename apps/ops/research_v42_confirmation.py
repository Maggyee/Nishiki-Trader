"""Validation for Protocol v42 confirmation holdout contract."""
...
DEFAULT_CONFIRMATION_PATH = Path("docs/progress/phase-2-research-v42-confirmation.json")
SCHEMA_VERSION = "research.protocol.v42.confirmation.v1"
...
def validate_confirmation(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if cand.get("key") != "vix6m_relief":
        raise ValueError(f"expected candidate vix6m_relief, got {cand.get('key')}")
    if cand.get("source") != "rule_cboe_vix6m_relief_v1":
        raise ValueError("source drifted")
    if cand.get("model_version") != "cboe-vix6m-diff5-negative-lag1d-v1":
