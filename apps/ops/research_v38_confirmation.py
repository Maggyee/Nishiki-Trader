"""Validate the frozen Protocol v38 confirmation contract."""
...
DEFAULT_CONTRACT = Path("docs/progress/phase-2-research-v38-confirmation.json")
DEVELOPMENT_RESULTS = Path("docs/progress/phase-2-research-v38-development-results.json")
SCHEMA_VERSION = "research.protocol.v38.confirmation.v1"
...
def validate_contract(payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("Protocol v38 confirmation schema drifted")
    if payload.get("status") != "preregistered_before_confirmation_open":
        raise ValueError("Protocol v38 confirmation status drifted")
    parent_validation = load_and_validate_parent()
    if payload.get("parent_protocol_sha256") != parent_validation["protocol_sha256"]:
        raise ValueError("Protocol v38 confirmation parent protocol fingerprint mismatch")
    dev_payload = json.loads(DEVELOPMENT_RESULTS.read_text())
    if dev_payload.get("schema_version") != "research.v38.development_results.v1":
        raise ValueError("Protocol v38 dev results schema drifted")
    candidate = payload.get("candidate", {})
    if candidate.get("key") != "cll_expansion":
        raise ValueError("candidate drifted; must be cll_expansion")
    source, model = IDENTITIES["cll_expansion"]
