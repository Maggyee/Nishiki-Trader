"""Apply frozen Protocol v28 gates to duplicate development bundles."""
...
from apps.ops.research_protocol_v28 import IDENTITIES, load_and_validate
from apps.ops.research_v13_review import build_standard_review

SCHEMA_VERSION = "research.v28.development_results.v1"
PROVIDER_QUALIFICATION = Path("docs/progress/phase-2-research-v28-provider-qualification.json")
...
def load_provider_qualification(
    ...
    if payload.get("schema_version") != "research.v28.provider_qualification.v1":
        raise ValueError("Protocol v28 provider qualification schema drifted")
    validation = load_and_validate()
    if payload.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v28 provider qualification fingerprint drifted")
    if payload.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v28 provider contract fingerprint drifted")
    qualified = payload.get("qualified_candidates")
    if not isinstance(qualified, list) or not qualified:
        raise ValueError("Protocol v28 has no provider-qualified candidate")
    return payload
...
def build_review(
    ...
    *,
    ...
    if any(key not in qualified for key, _ in candidate_specs):
        raise ValueError("Protocol v28 review includes a provider-rejected candidate")
