"""Validate the frozen Protocol v18 Cboe geographic/style volatility contract."""
...
DEFAULT_PROTOCOL = Path("docs/progress/phase-2-research-protocol-v18.json")
SCHEMA_VERSION = "research.protocol.v18"
IDENTITIES = {
    "em_vol_relief": ("rule_em_vol_relief_v2", "cboe-vxeem-ohlc5obs-negative-1d-v1"),
    "eafe_vol_relief": ("rule_eafe_vol_relief_v2", "cboe-vxefa-ohlc5obs-negative-1d-v1"),
    "nasdaq_vol_relief": ("rule_nasdaq_vol_relief_v2", "cboe-vxn-ohlc5obs-negative-1d-v1"),
}
PARAMETERS = {
...
def validate_protocol(payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("schema_version") != SCHEMA_VERSION or payload.get("status") != "preregistered_before_cboe_csv_body_access":
        raise ValueError("Protocol v18 pre-access identity drifted")
    if payload.get("data_access_disclosure", {}).get("csv_bodies_opened") is not False:
        raise ValueError("Protocol v18 must remain closed to CSV bodies")
    candidates = payload.get("candidates")
    if not isinstance(candidates, list) or len(candidates) != 3:
        raise ValueError("Protocol v18 must lock exactly three candidates")
    actual = {row["key"]: (row["source"], row["model_version"]) for row in candidates}
    if actual != IDENTITIES:
        raise ValueError("Protocol v18 candidate identities drifted")
    if any(row.get("parameters") != PARAMETERS or row.get("index") != INDEXES[row["key"]] for row in candidates):
        raise ValueError("Protocol v18 parameters or indexes drifted")
    ...
    if multiple.get("candidate_count_locked") != 3 or not all(
        ...
        raise ValueError("Protocol v18 multiple-testing guard drifted")
    if any(payload.get("boundaries_effect", {}).values()):
        raise ValueError("Protocol v18 boundaries are unsafe")
...
def load_and_validate(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Protocol v18 root must be an object")
