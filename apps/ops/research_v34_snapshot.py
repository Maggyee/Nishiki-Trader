"""Collect, verify, and extract Protocol v34 Cboe Treasury Yield raw snapshots."""
...
from apps.ops.research_protocol_v34 import PROVIDER_CONTRACT, load_and_validate

SCHEMA_VERSION = "research.raw_snapshot.v34"
KINDS = ("fvx", "tnx", "tyx")
...
def _request(kind: str) -> dict[str, Any]:
    if kind not in KINDS:
        raise ValueError(f"unsupported Protocol v34 kind {kind!r}")
...
def request_plan(kind: str) -> dict[str, Any]:
    spec = _request(kind)
    return {
        "schema_version": "research.snapshot.request_plan.v34",
        "kind": kind,
        "url": spec["url"],
...
def parse_and_audit_csv(kind: str, raw: bytes) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if development_dates[0] > date(2020, 1, 5):
        raise ValueError(f"{kind} development reserve starts too late")
    if development_dates[-1] < date(2022, 12, 28):
...
def verify_snapshot(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("invalid Protocol v34 snapshot schema")
    kind = str(envelope.get("kind"))
    spec = _request(kind)
    if envelope.get("url") != spec["url"] or envelope.get("index") != spec["index"]:
        raise ValueError("Protocol v34 snapshot request identity drifted")
    validation = load_and_validate()
    if envelope.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v34 protocol fingerprint mismatch")
    if envelope.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v34 provider fingerprint mismatch")
    if envelope.get("boundaries") != _boundaries():
        raise ValueError("Protocol v34 snapshot boundaries are unsafe")
    ...
    if envelope.get("payload_sha256") != "sha256:" + hashlib.sha256(raw).hexdigest():
        raise ValueError("Protocol v34 raw payload fingerprint mismatch")
    _, audit = parse_and_audit_csv(kind, raw)
    if envelope.get("audit") != audit:
        raise ValueError("Protocol v34 snapshot audit mismatch")
    core = {
        key: value
    ...
    if envelope.get("snapshot_sha256") != "sha256:" + digest or not str(
        ...
        raise ValueError("Protocol v34 snapshot fingerprint mismatch")
    return {
        "path": str(path),
        "kind": kind,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/research-v34/raw"))
