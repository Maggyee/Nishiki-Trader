...
STATUS_FILE = Path("docs/progress/phase-2-research-v48-paper-shadow-status.json")
...
def fetch_and_snapshot_basis(output_dir: Path) -> tuple[Path, dict[str, Any], list[dict[str, Any]]]:
    ...
    rows = fetch_index_mark_daily_range(years=years)
    if not rows:
        raise ValueError("failed to fetch basis rows")
    ...
    snapshot_path = output_dir / f"binance-vision-btc-basis-{observed_at.strftime('%Y%m%dT%H%M%SZ')}-{digest[:12]}.json"
...
def run_daily_shadow_collection(
    ...
    snapshot_path, envelope, rows = fetch_and_snapshot_basis(raw_dir)
    ...
    model = "crypto-btc-basis-below-ma10-lag1d-v1"
