"""Open the frozen v19 VXD holdout and export its point-in-time factor."""
...
from apps.ops.research_v19_snapshot import parse_and_audit_csv, verify_snapshot

SCHEMA_VERSION = "research.v19.confirmation_data.v1"
...
def _audit_confirmation_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(selected) < int(data["minimum_confirmation_rows"]):
        raise ValueError("Protocol v19 confirmation coverage is insufficient")
    days = [date.fromisoformat(row["date"]) for row in selected]
    if days[0] > date.fromisoformat(data["first_observation_on_or_before"]):
        raise ValueError("Protocol v19 confirmation starts too late")
    if days[-1] < date.fromisoformat(data["last_observation_on_or_after"]):
        raise ValueError("Protocol v19 confirmation ends too early")
    max_gap = max(
        (right - left).days
        for left, right in zip(days[:-1], days[1:], strict=True)
    )
    if max_gap > int(data["maximum_calendar_gap_days"]):
        raise ValueError("Protocol v19 confirmation gap exceeds frozen maximum")
    annual_counts = {
        str(year): sum(day.year == year for day in days)
...
def request_plan() -> dict[str, Any]:
    ...
    return {
        "schema_version": "research.v19.confirmation_factor_plan.v1",
...
def write_confirmation_factor(
    ...
    *,
    ...
    if snapshot_path != SNAPSHOT_PATH:
        raise ValueError("Protocol v19 confirmation snapshot path drifted")
    verification = verify_snapshot(snapshot_path)
    if verification["snapshot_sha256"] != SNAPSHOT_SHA256:
        raise ValueError("Protocol v19 confirmation snapshot fingerprint drifted")

    envelope = json.loads(snapshot_path.read_text())
    raw = base64.b64decode(envelope["payload_raw_base64"], validate=True)
    rows, _ = parse_and_audit_csv("vxd", raw)
    audit = _audit_confirmation_rows(rows)
    selected = [
        row
        for row in rows
        if WARMUP_START <= date.fromisoformat(row["date"]) <= CONFIRMATION_END
    ]
    warmup_rows = sum(
        date.fromisoformat(row["date"]) < CONFIRMATION_START for row in selected
    )
    if warmup_rows < 5:
        raise ValueError("Protocol v19 confirmation warmup is insufficient")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/research-v19/confirmation/factors/vxd.csv"),
    )
