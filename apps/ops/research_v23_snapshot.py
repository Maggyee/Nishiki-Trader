"""Collect immutable official Wikimedia pageview snapshots for Protocol v23."""
...
from apps.ops.research_protocol_v23 import PROVIDER_CONTRACT, load_and_validate

SCHEMA_VERSION = "research.raw_snapshot.v23"
KINDS = ("bitcoin", "ethereum", "cryptocurrency")
...
def _contract() -> dict[str, Any]:
    ...
    if payload.get("schema_version") != "research.data_sources.v23":
        raise ValueError("Protocol v23 provider schema drifted")
    return payload


def _request(kind: str) -> dict[str, Any]:
    if kind not in KINDS:
        raise ValueError(f"unsupported Protocol v23 kind {kind!r}")
...
def request_plan(kind: str) -> dict[str, Any]:
    spec = _request(kind)
    return {
        "schema_version": "research.snapshot.request_plan.v23",
        "kind": kind,
        "url": spec["url"],
...
def verify_snapshot(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("invalid Protocol v23 snapshot schema")
    kind = str(envelope.get("kind"))
    spec = _request(kind)
    if envelope.get("url") != spec["url"] or envelope.get("article") != spec["article"]:
        raise ValueError("Protocol v23 snapshot request identity drifted")
    validation = load_and_validate()
    if envelope.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v23 protocol fingerprint mismatch")
    if envelope.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v23 provider fingerprint mismatch")
    if envelope.get("boundaries") != _boundaries():
        raise ValueError("Protocol v23 snapshot boundaries are unsafe")
    ...
    if envelope.get("payload_sha256") != "sha256:" + hashlib.sha256(raw).hexdigest():
        raise ValueError("Protocol v23 raw payload fingerprint mismatch")
    _, audit = parse_and_audit_json(kind, raw)
    if envelope.get("audit") != audit:
        raise ValueError("Protocol v23 snapshot audit mismatch")
    core = {
        key: value
    ...
    if envelope.get("snapshot_sha256") != "sha256:" + digest or not str(
        ...
        raise ValueError("Protocol v23 snapshot fingerprint mismatch")
    return {
        "path": str(path),
        "kind": kind,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/research-v23/raw"))
