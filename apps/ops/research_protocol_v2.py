"""Validate and fingerprint the fail-closed Research Protocol v2 pre-registration."""
...
SCHEMA_VERSION = "research.protocol.v2"
DEFAULT_PROTOCOL = Path("docs/progress/phase-2-research-protocol-v2.json")
...
def validate_protocol(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if boundaries["opened_diagnostic_only"].get("pnl_selection_forbidden") is not True:
        raise ValueError("opened samples must forbid PnL selection")
    if boundaries["forward_pipeline_qualification"].get("pnl_access_forbidden") is not True:
        raise ValueError("pipeline qualification must forbid PnL access")
    if boundaries["historical_replication_reserve"].get("max_openings") != 1:
        raise ValueError("historical replication reserve must allow exactly one opening")
    if (
        boundaries["final_future_blind"].get("parameter_changes_after_open_forbidden")
        is not True
    ):
        raise ValueError("future blind must forbid post-open parameter changes")
    ...
    if payload.get("cost_scenarios") != expected_costs:
        raise ValueError("cost scenarios differ from the locked gross/base/stress values")

    execution = payload.get("shared_execution")
    if not isinstance(execution, dict):
        raise ValueError("shared_execution must be an object")
    locked_execution = {
        "venue": "BINANCE",
        "instrument": "BTCUSDT",
        "account_type": "CASH",
    ...
    if execution != locked_execution:
        raise ValueError("shared execution settings differ from the pre-registration")
    ...
    return {
        ...
        "candidate_identities": [
            {"key": candidate["key"], "source": candidate["source"], "model_version": candidate["model_version"]}
            for candidate in candidates
        ],
