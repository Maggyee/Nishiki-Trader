"""Validate the frozen Protocol v22 COR1M confirmation contract."""
...
from apps.ops.research_protocol_v22 import (
    load_and_validate as load_protocol,
)

DEFAULT_CONTRACT = Path("docs/progress/phase-2-research-v22-confirmation.json")
SCHEMA_VERSION = "research.protocol.v22.confirmation.v1"
CANDIDATE = "implied_correlation_relief"
DEVELOPMENT_REVIEW = "e6ea3a4"
DEVELOPMENT_RESULTS_PATH = Path("docs/progress/phase-2-research-v22-development-results.json")
DEVELOPMENT_RESULTS_SHA256 = (
    "sha256:f224dff76c713c7e0e51615072343d53226e4fbb738bd24e2fa378d17e5527a3"
)
PROTOCOL_SHA256 = "sha256:ae3edd20007a35470f94a57609e53f994bfb4cdf677318d99b6e3c249989685f"
SNAPSHOT_PATH = Path("data/research-v22/raw/cor1m-20260813T042019Z-2eeec6793d1d.json")
SNAPSHOT_SHA256 = "sha256:2eeec6793d1d43f7a3b962074cc9fc20bd5ced04302b8527a94cd6f7f01e55e0"
...
CANDIDATE_PARAMETERS = {**COMMON_PARAMETERS, "direction": "negative"}
COST_SCENARIOS = {
...
def validate_contract(payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("schema_version") != SCHEMA_VERSION or payload.get("status") != (
        "preregistered_before_cor1m_confirmation_value_access"
    ):
        raise ValueError("Protocol v22 confirmation identity drifted")
    ...
    if (
        ...
        raise ValueError("Protocol v22 development prerequisite drifted")
    ...
    if (
        ...
        raise ValueError("Protocol v22 confirmation access disclosure drifted")
    candidate = payload.get("candidate", {})
    if (
        (candidate.get("source"), candidate.get("model_version")) != IDENTITIES[CANDIDATE]
        or candidate.get("key") != CANDIDATE
        or candidate.get("index") != "COR1M"
        or candidate.get("parameters") != CANDIDATE_PARAMETERS
        or candidate.get("parameter_changes_forbidden") is not True
    ):
        raise ValueError("Protocol v22 confirmation candidate drifted")
    if payload.get("confirmation") != {
        ...
        raise ValueError("Protocol v22 confirmation partition drifted")
    data = payload.get("data_contract", {})
    if data.get("source_snapshot") != {
        "path": str(SNAPSHOT_PATH),
        "snapshot_sha256": SNAPSHOT_SHA256,
    }:
        raise ValueError("Protocol v22 confirmation snapshot drifted")
    if (
        ...
        raise ValueError("Protocol v22 confirmation data contract drifted")
    if (
        payload.get("cost_scenarios") != COST_SCENARIOS
        or payload.get("gates") != GATES
        or payload.get("outcomes") != OUTCOMES
    ):
        raise ValueError("Protocol v22 confirmation costs, gates, or outcomes drifted")
    if payload.get("future_blind") != {
        "start": "2026-09-01",
        "end": "2027-01-31",
        "status": "sealed_unopened",
    }:
        raise ValueError("Protocol v22 future blind drifted")
    if payload.get("boundaries_effect") != BOUNDARIES_EFFECT:
        raise ValueError("Protocol v22 confirmation boundaries are unsafe")
