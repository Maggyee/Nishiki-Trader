"""Contract definitions and validation for Phase 2 Alpha Research Protocol v43."""
...
DEFAULT_PROTOCOL_PATH = Path("docs/progress/phase-2-research-protocol-v43.json")
SCHEMA_VERSION = "research.protocol.v43.v1"
...
def validate_protocol(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(candidates) != set(IDENTITIES):
        raise ValueError(f"candidate mismatch: {sorted(candidates)} vs {sorted(IDENTITIES)}")
    for key, spec in candidates.items():
        ...
        if spec.get("parameters") != PARAMETERS[key]:
