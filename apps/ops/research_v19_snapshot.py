"""Collect immutable official Cboe snapshots for Protocol v19."""
...
from apps.ops.research_protocol_v19 import load_and_validate

PROVIDER_CONTRACT = Path("docs/progress/phase-2-research-v19-data-sources.json")
SCHEMA_VERSION = "research.raw_snapshot.v19"
KINDS = ("rvx", "vxd", "vxfxi")
...
def _contract() -> dict[str, Any]:
    ...
    if payload.get("schema_version") != "research.data_sources.v19":
        raise ValueError("v19 provider contract schema drifted")
    if payload.get("status") != "locked_before_cboe_csv_body_access":
        raise ValueError("v19 provider contract status drifted")
    if payload.get("authentication") != "none":
        raise ValueError("v19 provider must remain credential-free")
    requests = payload.get("requests")
    if not isinstance(requests, list) or [row.get("kind") for row in requests] != list(KINDS):
        raise ValueError("v19 provider contract must lock rvx, vxd, vxfxi in order")
    return payload


def _request(kind: str) -> dict[str, Any]:
    if kind not in KINDS:
        raise ValueError(f"unsupported v19 kind {kind!r}")
...
def request_plan(kind: str) -> dict[str, Any]:
    spec = _request(kind)
    return {
        "schema_version": "research.snapshot.request_plan.v19",
        "kind": kind,
        "url": spec["url"],
...
def _fetch(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "Nishiki-Trader/research-v19"})
...
def verify_snapshot(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("invalid Protocol v19 snapshot schema")
    kind = str(envelope.get("kind"))
    spec = _request(kind)
    if envelope.get("url") != spec["url"] or envelope.get("index") != spec["index"]:
        raise ValueError("Protocol v19 snapshot request identity drifted")
    if envelope.get("protocol_sha256") != load_and_validate()["protocol_sha256"]:
        raise ValueError("Protocol v19 protocol fingerprint mismatch")
    if envelope.get("boundaries") != _boundaries():
        raise ValueError("Protocol v19 snapshot boundaries are unsafe")
    ...
    if envelope.get("provider_contract_sha256") != expected_contract_hash:
        raise ValueError("Protocol v19 provider contract fingerprint mismatch")
    ...
    if envelope.get("payload_sha256") != "sha256:" + hashlib.sha256(raw).hexdigest():
        raise ValueError("Protocol v19 raw payload fingerprint mismatch")
    _, audit = parse_and_audit_csv(kind, raw)
    if envelope.get("audit") != audit:
        raise ValueError("Protocol v19 snapshot audit mismatch")
    core = {
        key: value
    ...
    if envelope.get("snapshot_sha256") != "sha256:" + digest or not str(
        ...
        raise ValueError("Protocol v19 snapshot fingerprint mismatch")
    return {
        "path": str(path),
        "kind": kind,
...
def write_factor_csv(snapshot_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "research.factor.v19",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/research-v19/raw"))
