"""Open frozen v22 COR1M holdout and export its point-in-time factor."""
...
from apps.ops.research_v22_snapshot import parse_and_audit_csv, verify_snapshot

SCHEMA_VERSION = "research.v22.confirmation_data.v1"
...
def _audit_confirmation_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(selected) < int(data["minimum_confirmation_rows"]):
        raise ValueError("Protocol v22 confirmation coverage is insufficient")
    days = [date.fromisoformat(row["date"]) for row in selected]
    if days[0] > date.fromisoformat(data["first_observation_on_or_before"]):
        raise ValueError("Protocol v22 confirmation starts too late")
    if days[-1] < date.fromisoformat(data["last_observation_on_or_after"]):
        raise ValueError("Protocol v22 confirmation ends too early")
    ...
    if maximum_gap > int(data["maximum_calendar_gap_days"]):
        raise ValueError("Protocol v22 confirmation gap exceeds frozen maximum")
    return {
...
def request_plan() -> dict[str, Any]:
    ...
    return {
        "schema_version": "research.v22.confirmation_factor_plan.v1",
...
def write_confirmation_factor(
    ...
    *,
    ...
    if warmup_rows < 5:
        raise ValueError("Protocol v22 confirmation warmup is insufficient")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/research-v22/confirmation/factors/cor1m.csv"),
    )
