"""Apply the frozen Protocol v35 gates to duplicate development bundles."""
...
from apps.ops.research_protocol_v35 import IDENTITIES, load_and_validate
from apps.ops.research_v13_review import build_standard_review

SCHEMA_VERSION = "research.v35.development_results.v1"
PROVIDER_QUALIFICATION = Path("docs/progress/phase-2-research-v35-provider-qualification.json")


def load_provider_qualification(path: Path = PROVIDER_QUALIFICATION) -> dict[str, Any]:
    payload = json.loads(path.read_text())
    if not isinstance(payload, dict) or payload.get("schema_version") != (
        "research.v35.provider_qualification.v1"
    ):
        raise ValueError("Protocol v35 provider qualification schema drifted")
    validation = load_and_validate()
    if payload.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v35 provider qualification protocol fingerprint mismatch")
    if payload.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v35 provider qualification contract fingerprint mismatch")
    return payload
