"""Collect one forward Protocol v36 VPN/BTC paper-shadow evidence attempt."""
...
from apps.ops.research_v36_snapshot import collect_snapshot, parse_and_audit_csv
from apps.strategies_freqtrade.research.option_strategy_signals import (
    IDENTITIES,
    generate_option_strategy_signals,
)

CONTRACT_PATH = Path("docs/progress/phase-2-research-v36-paper-shadow.json")
SCHEMA_VERSION = "research.v36.paper_shadow_attempt.v1"
STATUS_SCHEMA_VERSION = "research.v36.paper_shadow_status.v1"
...
def load_contract(path: Path = CONTRACT_PATH) -> dict[str, Any]:
    ...
    if contract.get("schema_version") != "research.v36.paper_shadow_contract.v1":
    ...
    if contract.get("candidate") != {
        "source": "rule_cboe_vpn_expansion_v1",
        "model_version": "cboe-vpn-diff5-positive-lag1d-v1",
        "candidate_key": "vpn_expansion",
        "index": "VPN",
        "symbol": "BTCUSDT",
        "venue": "BINANCE",
    }:
    ...
    if (
        collection.get("vpn_url")
        != "https://cdn.cboe.com/api/global/us_indices/daily_prices/VPN_History.csv"
        or collection.get("btc_url")
        != "https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=168"
        or collection.get("schedule") != "weekdays at 03:00 UTC"
        or collection.get("vpn_max_age_calendar_days") != 4
...
def _fetch(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "Nishiki-Trader/research-v36"})
    ...
    *,
    ...
    with lock_path.open("w") as lock:
        ...
        snapshot_path, snapshot = collect_snapshot(
            "vpn", output_dir=raw_dir / "vpn", fetch=fetch, now=observed_at
        )
        envelope = _strict_json(snapshot_path.read_bytes())
        vpn_raw = base64.b64decode(envelope["payload_raw_base64"], validate=True)
        vpn_rows, vpn_audit = parse_and_audit_csv("vpn", vpn_raw)
        ...
        vpn_state_path = state_dir / "vpn-observations.json"
        previous_vpn = _load_state(vpn_state_path)
        current_vpn = {row["date"]: format(float(row["value"]), ".12g") for row in vpn_rows}
        merged_vpn, vpn_revisions, new_vpn_rows = _merge_observations(previous_vpn, current_vpn)
        ...
        events = generate_option_strategy_signals(
            frame,
            candidate=contract["candidate"]["candidate_key"],
        ...
        source, model_version = IDENTITIES[contract["candidate"]["candidate_key"]]
        existing_ids = {
            event.signal_id
        ...
        last_vpn_date = date.fromisoformat(vpn_audit["last_date"])
        vpn_age_days = (observed_at.date() - last_vpn_date).days
        ...
        if vpn_age_days < 0 or vpn_age_days > contract["collection"]["vpn_max_age_calendar_days"]:
            blockers.append(f"vpn_stale:{vpn_age_days}_days")
        if vpn_revisions:
            blockers.append(f"vpn_historical_revision:{len(vpn_revisions)}")
        if btc_revisions:
        ...
        record = {
            ...
            "vpn": {
                ...
                "snapshot_sha256": snapshot["snapshot_sha256"],
                "vintage_id": snapshot["vintage_id"],
                "first_date": vpn_audit["first_date"],
                "last_date": vpn_audit["last_date"],
                "age_calendar_days": vpn_age_days,
                "new_observation_rows": new_vpn_rows,
                "historical_revisions": vpn_revisions,
            },
            "btc": {
                "url": btc_url,
        ...
        if not blockers:
            vpn_state_path.write_text(json.dumps(merged_vpn, indent=2, sort_keys=True) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--data-root", type=Path, default=Path("data/research-v36-forward"))
