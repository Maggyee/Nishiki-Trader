"""Validate the frozen Protocol v14 valuation-mechanism contract."""
...
DEFAULT_PROTOCOL = Path("docs/progress/phase-2-research-protocol-v14.json")
SCHEMA_VERSION = "research.protocol.v14"
IDENTITIES = {
...
def validate_protocol(payload: dict[str, Any]) -> dict[str, Any]:
    if (
        ...
        != "preregistered_before_coinmetrics_valuation_body_access"
    ):
        raise ValueError("Protocol v14 pre-access identity drifted")
    candidates = payload.get("candidates")
    if not isinstance(candidates, list) or len(candidates) != 3:
        raise ValueError("Protocol v14 must lock exactly three candidates")
    if {row["key"]: (row["source"], row["model_version"]) for row in candidates} != IDENTITIES:
        raise ValueError("Protocol v14 candidate identities drifted")
    if {row["key"]: row["parameters"] for row in candidates} != PARAMETERS:
        raise ValueError("Protocol v14 parameters drifted")
    ...
    if data.get("params") != {
        ...
        raise ValueError("Protocol v14 request parameters drifted")
    if data.get("decision_lag_days") != 2 or data.get("forward_fill_forbidden") is not True:
        raise ValueError("Protocol v14 information timing drifted")
    ...
    if multiple.get("candidate_count_locked") != 3 or not all(
        ...
        raise ValueError("Protocol v14 multiple-testing guard drifted")
    if any(payload.get("boundaries_effect", {}).values()):
        raise ValueError("Protocol v14 boundaries are unsafe")
...
def load_and_validate(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Protocol v14 root must be an object")
