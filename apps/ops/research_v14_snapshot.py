"""Collect one immutable qualification snapshot for Protocol v14."""
...
from apps.ops.research_protocol_v14 import DEFAULT_PROTOCOL, load_and_validate

SCHEMA_VERSION = "research.raw_snapshot.v14"
METRICS = ("CapRealUSD", "NVTAdj", "SOPR")
...
def _fetch(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "Nishiki-Trader/research-v14"})
...
def parse_and_audit(raw: bytes) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if payload.get("next_page_token") or payload.get("next_page_url"):
        raise ValueError("Protocol v14 response unexpectedly requires pagination")
    rows = payload["data"]
    seen: set[date] = set()
    ...
    for row in rows:
        if not isinstance(row, dict) or row.get("asset") != "btc":
            raise ValueError("Protocol v14 requires BTC rows")
        timestamp = datetime.fromisoformat(str(row.get("time")).replace("Z", "+00:00"))
        day = timestamp.date()
        if day in seen:
            raise ValueError("Protocol v14 daily rows must be unique")
        seen.add(day)
        ...
        for metric in METRICS:
            try:
                ...
                raise ValueError(f"Protocol v14 row is missing {metric}") from exc
            if not math.isfinite(value) or value <= 0.0:
                raise ValueError(f"Protocol v14 {metric} must be finite and positive")
    ...
    if first != expected_start or last != expected_end or len(rows) != expected_count:
        raise ValueError("Protocol v14 daily development grid is incomplete")
    return rows, {
        "row_count": len(rows),
    ...
    *, output_dir: Path, fetch: Fetch = _fetch, now: datetime | None = None
    ...
    envelope = {
        **core,
        ...
        "vintage_id": f"coinmetrics-btc-valuation:{retrieved.isoformat()}:{digest[:12]}",
    }
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / f"btc-valuation-{retrieved.strftime('%Y%m%dT%H%M%SZ')}-{digest[:12]}.json"
    if path.exists():
...
def verify_snapshot(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("invalid Protocol v14 snapshot schema")
    if envelope.get("url") != request_url():
        raise ValueError("Protocol v14 snapshot request identity drifted")
    if envelope.get("protocol_sha256") != load_and_validate()["protocol_sha256"]:
        raise ValueError("Protocol v14 protocol fingerprint mismatch")
    expected_boundaries = {
        "values_reported": False,
        "signals_generated": False,
        "pnl_opened": False,
    ...
    if envelope.get("boundaries") != expected_boundaries:
        raise ValueError("Protocol v14 snapshot boundaries are unsafe")
    ...
    if envelope.get("payload_sha256") != "sha256:" + hashlib.sha256(raw).hexdigest():
        raise ValueError("Protocol v14 raw payload fingerprint mismatch")
    _, audit = parse_and_audit(raw)
    if envelope.get("audit") != audit:
        raise ValueError("Protocol v14 snapshot audit mismatch")
    core = {
        key: value
    ...
    if envelope.get("snapshot_sha256") != "sha256:" + digest:
        raise ValueError("Protocol v14 snapshot fingerprint mismatch")
    if not str(envelope.get("vintage_id", "")).endswith(digest[:12]):
        raise ValueError("Protocol v14 snapshot vintage mismatch")
    return {
        "path": str(path),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/research-v14/raw"))
