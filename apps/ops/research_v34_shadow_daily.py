"""Collect one forward Protocol v34 FVX/BTC paper-shadow evidence attempt."""
...
from apps.ops.research_v34_snapshot import collect_snapshot, parse_and_audit_csv
from apps.strategies_freqtrade.research.treasury_yield_signals import (
    IDENTITIES,
    generate_treasury_yield_signals,
)

CONTRACT_PATH = Path("docs/progress/phase-2-research-v34-paper-shadow.json")
SCHEMA_VERSION = "research.v34.paper_shadow_attempt.v1"
STATUS_SCHEMA_VERSION = "research.v34.paper_shadow_status.v1"
...
def load_contract(path: Path = CONTRACT_PATH) -> dict[str, Any]:
    ...
    if contract.get("schema_version") != "research.v34.paper_shadow_contract.v1":
    ...
    if (
        collection.get("fvx_url")
        != "https://cdn.cboe.com/api/global/us_indices/daily_prices/FVX_History.csv"
        or collection.get("btc_url")
        != "https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=168"
        or collection.get("schedule") != "weekdays at 02:45 UTC"
        or collection.get("fvx_max_age_calendar_days") != 4
...
def _fetch(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "Nishiki-Trader/research-v34"})
    ...
    *,
    ...
    with lock_path.open("w") as lock:
        ...
        snapshot_path, snapshot = collect_snapshot(
            "fvx", output_dir=raw_dir / "fvx", fetch=fetch, now=observed_at
        )
        envelope = _strict_json(snapshot_path.read_bytes())
        fvx_raw = base64.b64decode(envelope["payload_raw_base64"], validate=True)
        fvx_rows, fvx_audit = parse_and_audit_csv("fvx", fvx_raw)
        ...
        fvx_state_path = state_dir / "fvx-observations.json"
        previous_fvx = _load_state(fvx_state_path)
        current_fvx = {row["date"]: format(float(row["value"]), ".12g") for row in fvx_rows}
        merged_fvx, fvx_revisions, new_fvx_rows = _merge_observations(previous_fvx, current_fvx)
        ...
        events = generate_treasury_yield_signals(
            frame,
        ...
        last_fvx_date = date.fromisoformat(fvx_audit["last_date"])
        fvx_age_days = (observed_at.date() - last_fvx_date).days
        ...
        if fvx_age_days < 0 or fvx_age_days > contract["collection"]["fvx_max_age_calendar_days"]:
            blockers.append(f"fvx_stale:{fvx_age_days}_days")
        if fvx_revisions:
            blockers.append(f"fvx_historical_revision:{len(fvx_revisions)}")
        if btc_revisions:
        ...
        record = {
            ...
            "fvx": {
                ...
                "snapshot_sha256": snapshot["snapshot_sha256"],
                "vintage_id": snapshot["vintage_id"],
                "first_date": fvx_audit["first_date"],
                "last_date": fvx_audit["last_date"],
                "age_calendar_days": fvx_age_days,
                "new_observation_rows": new_fvx_rows,
                "historical_revisions": fvx_revisions,
            },
            "btc": {
                "url": btc_url,
        ...
        if not blockers:
            fvx_state_path.write_text(json.dumps(merged_fvx, indent=2, sort_keys=True) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--data-root", type=Path, default=Path("data/research-v34-forward"))
