"""Collect immutable official DefiLlama TVL snapshots for Protocol v25."""
...
from apps.ops.research_protocol_v25 import PROVIDER_CONTRACT, load_and_validate

SCHEMA_VERSION = "research.raw_snapshot.v25"
KINDS = ("all_chains", "ethereum", "bitcoin")
...
def _contract() -> dict[str, Any]:
    ...
    if payload.get("schema_version") != "research.data_sources.v25":
        raise ValueError("Protocol v25 provider schema drifted")
    return payload


def _request(kind: str) -> dict[str, Any]:
    if kind not in KINDS:
        raise ValueError(f"unsupported Protocol v25 kind {kind!r}")
...
def request_plan(kind: str) -> dict[str, Any]:
    spec = _request(kind)
    return {
        "schema_version": "research.snapshot.request_plan.v25",
        "kind": kind,
        "url": spec["url"],
...
def _reject_constant(value: str) -> None:
    raise ValueError(f"DefiLlama JSON contains non-finite constant {value!r}")
...
def _tvl(value: Any, *, kind: str, index: int) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValueError(f"{kind} items[{index}] tvl must be numeric")
    result = float(value)
    if not math.isfinite(result) or result < 0.0:
        raise ValueError(f"{kind} items[{index}] tvl must be finite and non-negative")
    return result
...
def parse_and_audit_json(kind: str, raw: bytes) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    try:
        ...
        raise ValueError("DefiLlama TVL response must be UTF-8 JSON") from exc
    if not isinstance(payload, list) or not payload:
        raise ValueError(f"{kind} history is empty")
    parsed: list[dict[str, Any]] = []
    for index, item in enumerate(payload):
        if not isinstance(item, dict) or "date" not in item or "tvl" not in item:
            raise ValueError(f"{kind} items[{index}] is missing locked fields")
        observed = _parse_date(item.get("date"), kind=kind, index=index)
        parsed.append(
            {
                "date": observed.isoformat(),
                "value": _tvl(item.get("tvl"), kind=kind, index=index),
            }
        )
...
def verify_snapshot(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("invalid Protocol v25 snapshot schema")
    kind = str(envelope.get("kind"))
    spec = _request(kind)
    if envelope.get("url") != spec["url"]:
        raise ValueError("Protocol v25 snapshot request identity drifted")
    validation = load_and_validate()
    if envelope.get("protocol_sha256") != validation["protocol_sha256"]:
        raise ValueError("Protocol v25 protocol fingerprint mismatch")
    if envelope.get("provider_contract_sha256") != validation["provider_contract_sha256"]:
        raise ValueError("Protocol v25 provider fingerprint mismatch")
    if envelope.get("boundaries") != _boundaries():
        raise ValueError("Protocol v25 snapshot boundaries are unsafe")
    ...
    if envelope.get("payload_sha256") != "sha256:" + hashlib.sha256(raw).hexdigest():
        raise ValueError("Protocol v25 raw payload fingerprint mismatch")
    _, audit = parse_and_audit_json(kind, raw)
    if envelope.get("audit") != audit:
        raise ValueError("Protocol v25 snapshot audit mismatch")
    core = {
        key: value
    ...
    if envelope.get("snapshot_sha256") != "sha256:" + digest or not str(
        ...
        raise ValueError("Protocol v25 snapshot fingerprint mismatch")
    return {
        "path": str(path),
        "kind": kind,
            ...
            writer.writerow(
                {
                    ...
                    "vintage_id": envelope["vintage_id"],
                    "snapshot_sha256": envelope["snapshot_sha256"],
                    "tvl": format(float(row["value"]), ".12g"),
                }
            )
    return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/research-v25/raw"))
