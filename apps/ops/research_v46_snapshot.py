"""Fetch and audit Binance Vision Perpetual Premium Index daily klines for Protocol v46."""
...
from apps.ops.research_protocol_v46 import load_and_validate

SCHEMA_VERSION = "research.v46.snapshot.v1"
AUDIT_SCHEMA_VERSION = "research.v46.provider_qualification.v1"
RAW_ROOT = Path("data/research-v46/raw")
FACTORS_ROOT = Path("data/research-v46/factors")
QUALIFICATION_PATH = Path("docs/progress/phase-2-research-v46-provider-qualification.json")
DATA_SOURCES_PATH = Path("docs/progress/phase-2-research-v46-data-sources.json")
...
def compute_factors(premium_rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    ...
    factors_diff5_loose: list[dict[str, Any]] = []
    factors_diff5_minhold2: list[dict[str, Any]] = []
    ...
    for i, r in df.iterrows():
        ...
        # Candidate 2: diff5 < 0.00001
        c2 = bool(diff5.iloc[i] < 0.00001) if pd.notna(diff5.iloc[i]) else False
        factors_diff5_loose.append({"date": d, "value": 1.0 if c2 else -1.0})

        # Candidate 3: diff5 < 0.0 with min 2 days hold
        if c1 and not in_hold2:
            ...
            if not c1 and hold_cnt >= 2:
                in_hold2 = False
                hold_cnt = 0
        factors_diff5_minhold2.append({"date": d, "value": 1.0 if in_hold2 else -1.0})

    return {
        "btc_prem_diff5_negative": factors_diff5_neg,
        "btc_prem_diff5_loose": factors_diff5_loose,
        "btc_prem_diff5_minhold2": factors_diff5_minhold2,
    }
...
def collect_and_qualify_all(
    ...
    qualification = {
        ...
        "protocol": "docs/progress/phase-2-research-protocol-v46.json",
