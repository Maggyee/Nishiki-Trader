"""Open frozen v25 all-chain TVL holdout and export its point-in-time factor."""
...
from apps.ops.research_v25_snapshot import parse_all_json_rows, verify_snapshot

SCHEMA_VERSION = "research.v25.confirmation_data.v1"
...
def _audit_confirmation_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    data = json.loads(DEFAULT_CONTRACT.read_text())["data_contract"]
    selected = [
        row
        for row in rows
        if CONFIRMATION_START <= date.fromisoformat(row["date"]) <= CONFIRMATION_END
    ]
    ignored = [row for row in rows if date.fromisoformat(row["date"]) > CONFIRMATION_END]
    if len(selected) < int(data["minimum_confirmation_rows"]):
        raise ValueError("Protocol v25 confirmation coverage is insufficient")
    days = [date.fromisoformat(row["date"]) for row in selected]
    if days[0] > date.fromisoformat(data["first_observation_on_or_before"]):
        raise ValueError("Protocol v25 confirmation starts too late")
    if days[-1] < date.fromisoformat(data["last_observation_on_or_after"]):
        raise ValueError("Protocol v25 confirmation ends too early")
    maximum_gap = max((right - left).days for left, right in zip(days, days[1:], strict=False))
    if maximum_gap > int(data["maximum_calendar_gap_days"]):
        raise ValueError("Protocol v25 confirmation gap exceeds frozen maximum")
    return {
        ...
        "trailing_rows_ignored": len(ignored),
...
def request_plan() -> dict[str, Any]:
    ...
    return {
        "schema_version": "research.v25.confirmation_factor_plan.v1",
...
def write_confirmation_factor(
    ...
    *,
    ...
    if warmup_rows < 5:
        raise ValueError("Protocol v25 confirmation warmup is insufficient")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/research-v25/confirmation/factors/all_chains.csv"),
    )
