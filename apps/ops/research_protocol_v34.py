"""Validate the frozen Phase 2 Research Protocol v34 pre-registration contract."""
...
DEFAULT_PROTOCOL = Path("docs/progress/phase-2-research-protocol-v34.json")
PROVIDER_CONTRACT = Path("docs/progress/phase-2-research-v34-data-sources.json")
SCHEMA_VERSION = "research.protocol.v34"
...
IDENTITIES = {
    "fvx_relief": ("rule_cboe_fvx_relief_v1", "cboe-fvx-diff5-negative-lag1d-v1"),
    "tnx_relief": ("rule_cboe_tnx_relief_v1", "cboe-tnx-diff5-negative-lag1d-v1"),
    "tyx_relief": ("rule_cboe_tyx_relief_v1", "cboe-tyx-diff5-negative-lag1d-v1"),
}
...
def _validate_provider_contract(payload: dict[str, Any]) -> str:
    if payload.get("schema_version") != "research.data_sources.v34":
        raise ValueError("Protocol v34 provider schema drifted")
    if payload.get("provider") != "Cboe Global Indices":
        raise ValueError("Protocol v34 provider must be Cboe Global Indices")
    requests = payload.get("requests", [])
    if len(requests) != 3:
        raise ValueError("Protocol v34 requires exactly three provider requests")
    by_kind = {req.get("kind"): req for req in requests}
    if set(by_kind.keys()) != set(INDEX_BY_KIND.keys()):
        raise ValueError("Protocol v34 request kinds drifted")
    for kind, expected_idx in INDEX_BY_KIND.items():
        req = by_kind[kind]
        if req.get("index") != expected_idx:
            raise ValueError(f"Protocol v34 {kind} index drifted")
        if req.get("url") != f"https://cdn.cboe.com/api/global/us_indices/daily_prices/{expected_idx}_History.csv":
            raise ValueError(f"Protocol v34 {kind} URL drifted")
        ...
        if (
            ...
            raise ValueError(f"Protocol v34 {kind} coverage gates drifted")
...
def validate_protocol(
    ...
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("Protocol v34 schema drifted")
    if payload.get("status") != "preregistered_before_provider_access":
        raise ValueError("Protocol v34 status drifted")
    if payload.get("provider_contract") != str(PROVIDER_CONTRACT):
        raise ValueError("Protocol v34 provider contract path drifted")
    if payload.get("parameter_lock") != {
        ...
        raise ValueError("Protocol v34 parameter lock drifted")
    data_contract = payload.get("data_contract", {})
    for key, value in LOCKED_WINDOWS.items():
        if data_contract.get(key) != value:
            raise ValueError(f"Protocol v34 {key} drifted")
    if data_contract.get("publication_lag_calendar_days") != 1:
        raise ValueError("Protocol v34 publication lag must be 1 day")
    if data_contract.get("historical_vintage_claim") is not False:
        raise ValueError("Protocol v34 must not make historical vintage claim")
    if payload.get("execution_contract") != LOCKED_EXECUTION:
        raise ValueError("Protocol v34 execution contract drifted")
    if payload.get("cost_scenarios") != COST_SCENARIOS:
        raise ValueError("Protocol v34 cost scenarios drifted")
    if payload.get("gates") != GATES:
        raise ValueError("Protocol v34 gates drifted")
    if payload.get("boundaries") != BOUNDARIES:
        raise ValueError("Protocol v34 boundaries drifted")
    candidates = payload.get("candidates", [])
    if len(candidates) != 3:
        raise ValueError("Protocol v34 requires exactly three candidates")
    by_key = {c.get("key"): c for c in candidates}
    if set(by_key.keys()) != set(IDENTITIES.keys()):
        raise ValueError("Protocol v34 candidate keys drifted")
    for key, (expected_source, expected_model) in IDENTITIES.items():
        ...
        if c.get("source") != expected_source or c.get("model_version") != expected_model:
            raise ValueError(f"Protocol v34 {key} identity drifted")
        if c.get("parameters") != COMMON_PARAMETERS:
            raise ValueError(f"Protocol v34 {key} parameters drifted")
...
def load_and_validate(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Protocol v34 payload must be a dictionary")
