...
PROTOCOL_VERSION = "research.protocol.v39"
...
def audit_factor_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Protocol v39 factor frame is missing columns: {missing}")
    if not isinstance(frame.index, pd.DatetimeIndex) or frame.index.tz is None:
        raise ValueError("Protocol v39 factor timestamps must be timezone-aware")
    if frame.empty or frame.index.has_duplicates or not frame.index.is_monotonic_increasing:
        raise ValueError("Protocol v39 factor timestamps must be unique and increasing")
    audited = frame.copy()
    ...
    if (available.array.asi8 > audited.index.asi8).any():
        raise ValueError("Protocol v39 available_at after ts_event would introduce lookahead")
    if not ((available - observed) == pd.Timedelta(days=1)).all():
        raise ValueError("Protocol v39 requires the frozen one-day publication lag")
    hashes = audited["snapshot_sha256"].astype(str)
    if not hashes.map(lambda value: bool(_SHA256_RE.fullmatch(value))).all():
        raise ValueError("Protocol v39 snapshot hashes are invalid")
    if audited["vintage_id"].astype(str).str.strip().eq("").any():
        raise ValueError("Protocol v39 vintage identifiers cannot be empty")
    values = pd.to_numeric(audited["index_value"], errors="raise").astype("float64")
    if not values.map(math.isfinite).all():
        raise ValueError("Protocol v39 index values must be finite")
...
def generate_low_vol_harvest_signals(
    ...
    *,
    ...
    if direction != "positive":
        raise ValueError("Protocol v39 requires positive change rule")
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = generate_low_vol_harvest_signals(
