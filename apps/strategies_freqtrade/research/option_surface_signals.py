"""SignalEvent generators for Protocol v22 option-surface mechanisms."""
...
PROTOCOL_VERSION = "research.protocol.v22"
...
def audit_factor_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Protocol v22 factor frame is missing columns: {missing}")
    if not isinstance(frame.index, pd.DatetimeIndex) or frame.index.tz is None:
        raise ValueError("Protocol v22 factor timestamps must be timezone-aware")
    if frame.empty or frame.index.has_duplicates or not frame.index.is_monotonic_increasing:
        raise ValueError("Protocol v22 factor timestamps must be unique and increasing")
    audited = frame.copy()
    ...
    if (available.array.asi8 > audited.index.asi8).any():
        raise ValueError("Protocol v22 available_at after ts_event would introduce lookahead")
    if not ((available - observed) == pd.Timedelta(days=1)).all():
        raise ValueError("Protocol v22 requires the frozen one-day publication lag")
    hashes = audited["snapshot_sha256"].astype(str)
    if not hashes.map(lambda value: bool(_SHA256_RE.fullmatch(value))).all():
        raise ValueError("Protocol v22 snapshot hashes are invalid")
    if audited["vintage_id"].astype(str).str.strip().eq("").any():
        raise ValueError("Protocol v22 vintage identifiers cannot be empty")
    values = pd.to_numeric(audited["index_value"], errors="raise").astype("float64")
    if not values.map(math.isfinite).all():
        raise ValueError("Protocol v22 index values must be finite")
...
def generate_option_surface_signals(
    ...
    *,
    ...
    if candidate not in IDENTITIES:
        raise ValueError(f"unsupported Protocol v22 candidate {candidate!r}")
    frame = audit_factor_frame(factors)
    values = frame["index_value"]
    if candidate in {"tail_skew_relief", "implied_dispersion_expansion"} and (values <= 0).any():
        raise ValueError(f"Protocol v22 {candidate} values must be positive")
    change = values.diff(int(COMMON_PARAMETERS["change_observations"]))
    ready = change.notna()
    direction = DIRECTION_BY_CANDIDATE[candidate]
    states = (
        (change < float(COMMON_PARAMETERS["threshold"]))
        if direction == "negative"
        else (change > float(COMMON_PARAMETERS["threshold"]))
    ).where(ready)
            ...
            json.dumps(
                {
                    ...
                    "candidate": candidate,
                    "index": INDEX_BY_CANDIDATE[candidate],
                    "direction": direction,
    ...
    for timestamp, desired in states.items():
        ...
        favorable_change = -change_value if direction == "negative" else change_value
            ...
            SignalEvent.model_validate(
                {
                    ...
                    "horizon": "1d",
                    "side": side,
                    "score": math.tanh(max(0.0, favorable_change) / 10.0) if desired_long else 0.0,
                    ...
                    "metadata": {
                        ...
                        "point_in_time_interpretation": "cboe_current_history_lagged_1_calendar_day",
                        "trigger": f"{candidate}_5obs_{direction}"
                        if desired_long
                        else f"{candidate}_not_{direction}",
                        "inputs": {
                            "index_value": round(float(values.loc[timestamp]), 12),
