"""Signal generators for Protocol v13 BTC network mechanisms."""
...
METRIC_COLUMNS = {
    "active_address_expansion": "AdrActCnt",
    "transfer_count_expansion": "TxTfrCnt",
    "mvrv_distress": "CapMVRVCur",
}
...
def generate_network_signals(
    ...
    *,
    ...
    if frame.empty or frame.index.has_duplicates or metric_name not in frame:
        raise ValueError("Protocol v13 factor frame is invalid")
    values = pd.to_numeric(frame[metric_name], errors="raise")
    if not values.map(math.isfinite).all() or (values <= 0).any():
        raise ValueError("Protocol v13 metric values must be finite and positive")
    params = PARAMETERS[candidate]
    if candidate == "mvrv_distress":
        comparison = 1.0 - values
        states = values < float(params["maximum_mvrv"])
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = generate_network_signals(build_factor_frame(args.snapshot), candidate=args.candidate)
