"""SignalEvent generators for Protocol v27 DefiLlama protocol-fee-expansion mechanisms."""
...
from apps.ops.research_protocol_v27 import (
    COMMON_PARAMETERS,
    IDENTITIES,
    KIND_BY_CANDIDATE,
)
...
PROTOCOL_VERSION = "research.protocol.v27"
...
def audit_factor_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Protocol v27 factor frame is missing columns: {missing}")
    if not isinstance(frame.index, pd.DatetimeIndex) or frame.index.tz is None:
        raise ValueError("Protocol v27 factor timestamps must be timezone-aware")
    if frame.empty or frame.index.has_duplicates or not frame.index.is_monotonic_increasing:
        raise ValueError("Protocol v27 factor timestamps must be unique and increasing")
    audited = frame.copy()
    ...
    if (available.array.asi8 > audited.index.asi8).any():
        raise ValueError("Protocol v27 available_at after ts_event would introduce lookahead")
    if not ((available - observed) == pd.Timedelta(days=2)).all():
        raise ValueError("Protocol v27 requires the frozen two-day publication lag")
    hashes = audited["snapshot_sha256"].astype(str)
    if not hashes.map(lambda value: bool(_SHA256_RE.fullmatch(value))).all():
        raise ValueError("Protocol v27 snapshot hashes are invalid")
    if audited["vintage_id"].astype(str).str.strip().eq("").any():
        raise ValueError("Protocol v27 vintage identifiers cannot be empty")
    values = pd.to_numeric(audited["amount"], errors="raise").astype("float64")
    if not values.map(math.isfinite).all() or (values < 0).any():
        raise ValueError("Protocol v27 fee values must be finite and non-negative")
    audited["available_at"] = available
    audited["observation_date"] = observed
    audited["amount"] = values
    return audited


def generate_protocol_fee_expansion_signals(
    ...
    *,
    ...
    if candidate not in IDENTITIES:
        raise ValueError(f"unsupported Protocol v27 candidate {candidate!r}")
            ...
            SignalEvent.model_validate(
                {
                    ...
                    "metadata": {
                        ...
                        "point_in_time_interpretation": (
                            "defillama_current_protocol_fee_lagged_2_calendar_days"
                        ),
                        "trigger": (
                        ...
                        "inputs": {
                            "amount": round(float(values.loc[timestamp]), 12),
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = generate_protocol_fee_expansion_signals(
