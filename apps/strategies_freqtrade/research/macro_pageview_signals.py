"""SignalEvent generators for Protocol v29 Wikipedia macro-attention-relief mechanisms."""
...
from apps.ops.research_protocol_v29 import (
...
PROTOCOL_VERSION = "research.protocol.v29"
...
def audit_factor_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Protocol v29 factor frame is missing columns: {missing}")
    if not isinstance(frame.index, pd.DatetimeIndex) or frame.index.tz is None:
        raise ValueError("Protocol v29 factor timestamps must be timezone-aware")
    if frame.empty or frame.index.has_duplicates or not frame.index.is_monotonic_increasing:
        raise ValueError("Protocol v29 factor timestamps must be unique and increasing")
    audited = frame.copy()
    ...
    if (available.array.asi8 > audited.index.asi8).any():
        raise ValueError("Protocol v29 available_at after ts_event would introduce lookahead")
    if not ((available - observed) == pd.Timedelta(days=2)).all():
        raise ValueError("Protocol v29 requires the frozen two-day publication lag")
    hashes = audited["snapshot_sha256"].astype(str)
    if not hashes.map(lambda value: bool(_SHA256_RE.fullmatch(value))).all():
        raise ValueError("Protocol v29 snapshot hashes are invalid")
    if audited["vintage_id"].astype(str).str.strip().eq("").any():
        raise ValueError("Protocol v29 vintage identifiers cannot be empty")
    values = pd.to_numeric(audited["pageviews"], errors="raise").astype("float64")
    if not values.map(math.isfinite).all() or (values < 0).any():
        raise ValueError("Protocol v29 pageviews must be finite and non-negative")
    audited["available_at"] = available
    audited["observation_date"] = observed
    audited["pageviews"] = values
    return audited


def generate_macro_pageview_relief_signals(
    ...
    *,
    ...
    if candidate not in IDENTITIES:
        raise ValueError(f"unsupported Protocol v29 candidate {candidate!r}")
    frame = audit_factor_frame(factors)
    values = frame["pageviews"]
            ...
            json.dumps(
                {
                    ...
                    "candidate": candidate,
                    "article": ARTICLE_BY_CANDIDATE[candidate],
            ...
            SignalEvent.model_validate(
                {
                    ...
                    "horizon": "1d",
                    "side": side,
                    "score": math.tanh(max(0.0, -change_value) / 10000.0) if desired_long else 0.0,
                    ...
                    "metadata": {
                        ...
                        "point_in_time_interpretation": (
                            "wikimedia_current_pageviews_lagged_2_calendar_days"
                        ),
                        "trigger": (
                        ...
                        "inputs": {
                            "pageviews": round(float(values.loc[timestamp]), 12),
