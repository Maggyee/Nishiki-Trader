"""SignalEvent generators for Protocol v31 Cboe commodity/sector/duration volatility-relief mechanisms."""
...
from apps.ops.research_protocol_v31 import COMMON_PARAMETERS, IDENTITIES, INDEX_BY_CANDIDATE
...
PROTOCOL_VERSION = "research.protocol.v31"
...
def audit_factor_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Protocol v31 factor frame is missing columns: {missing}")
    if not isinstance(frame.index, pd.DatetimeIndex) or frame.index.tz is None:
        raise ValueError("Protocol v31 factor timestamps must be timezone-aware")
    if frame.empty or frame.index.has_duplicates or not frame.index.is_monotonic_increasing:
        raise ValueError("Protocol v31 factor timestamps must be unique and increasing")
    audited = frame.copy()
    ...
    if (available.array.asi8 > audited.index.asi8).any():
        raise ValueError("Protocol v31 available_at after ts_event would introduce lookahead")
    if not ((available - observed) == pd.Timedelta(days=1)).all():
        raise ValueError("Protocol v31 requires the frozen one-day publication lag")
    hashes = audited["snapshot_sha256"].astype(str)
    if not hashes.map(lambda value: bool(_SHA256_RE.fullmatch(value))).all():
        raise ValueError("Protocol v31 snapshot hashes are invalid")
    if audited["vintage_id"].astype(str).str.strip().eq("").any():
        raise ValueError("Protocol v31 vintage identifiers cannot be empty")
    values = pd.to_numeric(audited["index_value"], errors="raise").astype("float64")
    if not values.map(math.isfinite).all() or (values <= 0).any():
        raise ValueError("Protocol v31 index values must be finite and strictly positive")
...
def generate_commodity_sector_duration_vol_relief_signals(
    ...
    *,
    ...
    if candidate not in IDENTITIES:
        raise ValueError(f"unsupported Protocol v31 candidate {candidate!r}")
            ...
            SignalEvent.model_validate(
                {
                    ...
                    "horizon": "1d",
                    "side": side,
                    "score": math.tanh(max(0.0, -change_value) / 10.0) if desired_long else 0.0,
                    ...
                    "metadata": {
                        ...
                        "point_in_time_interpretation": (
                            "cboe_current_history_lagged_1_calendar_day"
                        ),
                        "trigger": (
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = generate_commodity_sector_duration_vol_relief_signals(
