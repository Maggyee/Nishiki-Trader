"""Signal generators for Protocol v16 Treasury-direct rate mechanisms."""
...
from apps.ops.research_protocol_v16 import IDENTITIES, PARAMETERS, load_and_validate
from apps.ops.research_v16_snapshot import combine_and_audit, request_specs, verify_snapshot
from apps.strategies_freqtrade.research.freqai_linear_signals import _make_signal_id

PROTOCOL_VERSION = "research.protocol.v16"
METRIC_COLUMNS = {
    "real_yield_relief": "real_10y",
    "yield_curve_steepening": "nominal_10y_minus_2y",
    "treasury_volatility_relief": "nominal_10y",
}
...
def build_factor_frame(snapshot_path: Path) -> pd.DataFrame:
    ...
    payloads = []
    for spec, record in zip(request_specs(), envelope["requests"], strict=True):
...
def generate_treasury_rate_signals(
    frame: pd.DataFrame,
    *,
    candidate: str,
    ...
    if candidate not in IDENTITIES:
        raise ValueError(f"unsupported Protocol v16 candidate {candidate!r}")
    metric_name = METRIC_COLUMNS[candidate]
    if frame.empty or frame.index.has_duplicates or metric_name not in frame:
        raise ValueError("Protocol v16 factor frame is invalid")
    values = pd.to_numeric(frame[metric_name], errors="raise")
    if not values.map(math.isfinite).all():
        raise ValueError("Protocol v16 metric values must be finite")
    ...
    feature_hash = "sha256:" + hashlib.sha256(
        json.dumps(
            ...
            SignalEvent.model_validate(
                {
                    ...
                    "metadata": {
                        ...
                        "point_in_time_interpretation": "treasury_reconstruction_lag2_calendar_days",
                        **params,
                    },
                }
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = generate_treasury_rate_signals(
        build_factor_frame(args.snapshot), candidate=args.candidate
    )
