"""SignalEvent generators for Protocol v19 Cboe size/style/China volatility."""
...
PROTOCOL_VERSION = "research.protocol.v19"
_SHA256_RE = re.compile(r"sha256:[0-9a-f]{64}")
STRATEGY_IDENTITIES = {
    "russell_vol_relief": (*IDENTITIES["russell_vol_relief"], "RVX"),
    "dow_vol_relief": (*IDENTITIES["dow_vol_relief"], "VXD"),
    "china_vol_relief": (*IDENTITIES["china_vol_relief"], "VXFXI"),
}
...
class VolatilityReliefParams:
    ...
    def __post_init__(self) -> None:
        if asdict(self) != PARAMETERS:
            raise ValueError("Protocol v19 volatility-relief parameters cannot be tuned")
...
def generate_size_style_vol_signals(
    ...
    *,
    ...
    if strategy not in STRATEGY_IDENTITIES:
        raise ValueError(f"unsupported Protocol v19 strategy {strategy!r}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = generate_size_style_vol_signals(
